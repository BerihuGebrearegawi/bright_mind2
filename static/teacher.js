...
async function loadCourses(){
 ...
 $('courseList').innerHTML=courses.length?courses.map(c=>`<div class="list-row"><div><strong>${esc(c.title)}</strong><div class="subtitle">Grade ${esc(c.className)}${c.subject?' • '+esc(c.subject):''} • ${esc(c.description||'')} • Mode: ${esc((c.learningModes||['Regular']).join(', '))} • Audience: ${esc((c.audiences||[]).join(', '))}</div></div><span class="badge badge-free">Published</span></div>`).join(''):'<p class="subtitle">No courses yet.</p>';
...
$('courseForm').addEventListener('submit',async e=>{e.preventDefault();try{const lm=[...$('courseLearningMode').selectedOptions].map(o=>o.value);const aud=[...$('courseAudience').selectedOptions].map(o=>o.value);await api('/api/teacher/courses',{method:'POST',body:JSON.stringify({title:$('courseTitle').value.trim(),className:$('courseClass').value,description:$('courseDescription').value.trim(),subject:$('courseSubject').value.trim(),learningModes:lm,audiences:aud})});e.target.reset();await loadCourses();toast('Course created','success')}catch(err){toast(err.message,'error')}});
$('lessonForm').addEventListener('submit',async e=>{e.preventDefault();try{await api('/api/teacher/lessons',{method:'POST',body:JSON.stringify({courseId:$('lessonCourse').value,title:$('lessonTitle').value.trim(),contentType:$('lessonType').value,url:$('lessonUrl').value.trim(),description:$('lessonDescription').value.trim(),order:parseInt($('lessonOrder').value||'0',10)||0})});e.target.reset();toast('Lesson published','success')}catch(err){toast(err.message,'error')}});
...
async function forgotPinTeacher(){
    // V31.108 fix: mirrors forgotPinStudent() in student.js -- teacher
    // phone+PIN accounts had no PIN-recovery path either. Firebase Phone
    // Auth OTP proves phone ownership (signs into the same uid), then
    // /api/auth/phone-pin/reset sets the new PIN.
    ...
    try{
        ...
        if(!newPin||!/^[0-9]{6,8}$/.test(newPin)){toast('PIN must be 6–8 digits.','error');return;}
        ...
        toast('PIN reset. Sign in with your new PIN.','success');
    }catch(e){toast(e.message||'Could not reset PIN.','error');}
}
window.forgotPinTeacher=forgotPinTeacher;
...
onAuthStateChanged(auth,async u=>{
 ...
 teacher=u;window.dispatchEvent(new Event('BMTAuthReady'));if($('teacherHeader')) $('teacherHeader').style.display=u?'flex':'none';if(u){const bt=document.getElementById('bmtLandingTop');if(bt)bt.style.display='none';const gr=document.getElementById('teacherGuestRedirect');if(gr)gr.style.display='none';}if(window.BMTScannerUI){window.BMT_SCANNER_CONFIG={getToken:()=>teacher?.getIdToken(true)};window.BMTScannerUI.init();}if(!u){location.href='/auth?role=teacher&next='+encodeURIComponent(location.pathname);}else{await load();if(BMT_PREVIEW_ROLE) applyTeacherPreviewSafeguards();}
});
...
async function loadAiMaterials(){
 try{
  ...
  $('aiMaterialList').innerHTML=list.length?list.map(m=>{const tags=[...(m.learningModes||[]),...(m.audiences||[])];return `<div class="list-row"><div><strong>${esc(m.title)}</strong><div class="subtitle">Grade ${esc(m.grade)} • ${esc(m.subject)} • ${esc(m.sourceType)} • ${m.chunkCount||0} chunks${tags.length?' • 🎯 '+tags.map(esc).join(', '):''}</div></div><button class="btn btn-danger" data-ai-delete="${esc(m.id)}">Delete</button></div>`}).join(''):'<p class="subtitle">No AI materials yet.</p>';
...
$('aiMaterialForm').addEventListener('submit',async e=>{
 ...
 [...(document.getElementById('aiMaterialLearningMode')?.selectedOptions||[])].forEach(o=>f.append('learningModes',o.value));
 [...(document.getElementById('aiMaterialAudience')?.selectedOptions||[])].forEach(o=>f.append('audiences',o.value));
 try{
...
// ==========================================================
// ✅ Classroom Attendance (Regular students only). Server enforces
// grade-ownership and Regular-vs-Distance separation; this UI just
// renders the roster/status the API returns and posts back edits.
// ==========================================================
async function loadAttendanceRoster(){
 const grade=$('attendanceGrade').value, subject=$('attendanceSubject').value.trim(), date=$('attendanceDate').value||new Date().toISOString().slice(0,10);
 const list=$('attendanceRosterList'), status=$('attendanceStatus'), saveBtn=$('saveAttendanceBtn');
 if(!grade){toast('Choose a grade first','error');return}
 if(!$('attendanceDate').value)$('attendanceDate').value=date;
 list.innerHTML='<p class="subtitle">Loading roster…</p>'; if(status)status.textContent=''; if(saveBtn)saveBtn.style.display='none';
 try{
  const q=new URLSearchParams({className:grade,date}); if(subject)q.set('subject',subject);
  const d=await api('/api/teacher/attendance?'+q.toString());
  const rows=d.students||[];
  list.innerHTML=rows.length?('<table style="width:100%;min-width:420px"><thead><tr><th>Student</th><th>Status</th></tr></thead><tbody>'+rows.map(s=>`<tr><td>${esc(s.name)}</td><td><select data-attendance-uid="${esc(s.uid)}"><option value="present"${s.status==='present'?' selected':''}>Present</option><option value="absent"${s.status==='absent'?' selected':''}>Absent</option><option value="late"${s.status==='late'?' selected':''}>Late</option></select></td></tr>`).join('')+'</tbody></table>'):'<p class="subtitle">No Regular students found for this grade.</p>';
  if(saveBtn)saveBtn.style.display=rows.length?'inline-block':'none';
 }catch(e){list.innerHTML=`<p class="error-text">${esc(e.message)}</p>`}
}
document.getElementById('loadAttendanceRosterBtn')?.addEventListener('click',loadAttendanceRoster);
document.getElementById('saveAttendanceBtn')?.addEventListener('click',async()=>{
 const grade=$('attendanceGrade').value, subject=$('attendanceSubject').value.trim(), date=$('attendanceDate').value;
 const status=$('attendanceStatus');
 const records=[...document.querySelectorAll('[data-attendance-uid]')].map(sel=>({studentUid:sel.dataset.attendanceUid,status:sel.value}));
 if(!records.length){toast('Load a roster first','error');return}
 try{
  const d=await api('/api/teacher/attendance',{method:'POST',body:JSON.stringify({className:grade,subject,date,records})});
  if(status)status.textContent=`✅ Saved ${d.markedCount} record(s).`+(d.skipped?.length?` ${d.skipped.length} skipped (see console).`:'');
  if(d.skipped?.length)console.warn('Attendance skipped:',d.skipped);
  toast('Attendance saved','success');
 }catch(e){if(status)status.textContent='❌ '+e.message; toast(e.message,'error')}
});
document.getElementById('loadMarkListBtn')?.addEventListener('click',async()=>{
 const grade=$('markListGrade').value, box=$('markListResults');
 if(!grade){toast('Choose a grade first','error');return}
 box.innerHTML='<p class="subtitle">Loading mark list…</p>';
 try{
  const d=await api('/api/teacher/marklist?className='+encodeURIComponent(grade));
  const rows=d.students||[];
  box.innerHTML=rows.length?('<table style="width:100%;min-width:560px"><thead><tr><th>Student</th><th>Assignment avg</th><th>Exam avg</th><th>Overall</th></tr></thead><tbody>'+rows.map(r=>`<tr><td>${esc(r.name)}</td><td>${r.assignmentAverage!=null?r.assignmentAverage+'% ('+r.assignmentCount+')':'—'}</td><td>${r.examAverage!=null?r.examAverage+'% ('+r.examCount+')':'—'}</td><td><strong>${r.overallAverage!=null?r.overallAverage+'%':'—'}</strong></td></tr>`).join('')+'</tbody></table>'):'<p class="subtitle">No graded assignments or exams found for this grade yet.</p>';
 }catch(e){box.innerHTML=`<p class="error-text">${esc(e.message)}</p>`}
});
document.getElementById('loadAttendanceReportBtn')?.addEventListener('click',async()=>{
 const grade=$('attendanceReportGrade').value, from=$('attendanceReportFrom').value, to=$('attendanceReportTo').value;
 const box=$('attendanceReportResults');
 if(!grade){toast('Choose a grade first','error');return}
 box.innerHTML='<p class="subtitle">Loading report…</p>';
 try{
  const q=new URLSearchParams({className:grade}); if(from)q.set('from',from); if(to)q.set('to',to);
  const d=await api('/api/teacher/attendance/report?'+q.toString());
  const rows=d.students||[];
  box.innerHTML=rows.length?(`<p class="subtitle">${esc(d.from)} → ${esc(d.to)}</p><table style="width:100%;min-width:520px"><thead><tr><th>Student</th><th>Present</th><th>Absent</th><th>Late</th><th>Rate</th></tr></thead><tbody>`+rows.map(r=>`<tr><td>${esc(r.name)}</td><td>${r.present}</td><td>${r.absent}</td><td>${r.late}</td><td>${r.attendanceRate!=null?r.attendanceRate+'%':'—'}</td></tr>`).join('')+'</tbody></table>'):'<p class="subtitle">No attendance recorded for this range.</p>';
 }catch(e){box.innerHTML=`<p class="error-text">${esc(e.message)}</p>`}
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    attachListFilter('lessonPlanListFilter', 'lessonPlanList');
});

// ==========================================================
// 🗓️ V31.108 — Teacher Lesson Plan. Mirrors the existing
// Assignments card's pattern exactly: same api() helper, same
// "chain onto load()" hook so the real load() function is never
// touched, same list-row markup, same inline-select-triggers-PATCH
// pattern already used for attendance status.
// ==========================================================
async function loadTeacherLessonPlans(){
 const list=$('lessonPlanList'); if(!list)return;
 try{
  const d=await api('/api/teacher/lesson-plans'); const rows=d.lessonPlans||[];
  const statusLabel={planned:'Planned',in_progress:'In Progress',completed:'Completed'};
  list.innerHTML=rows.length?rows.map(p=>`<div class="list-row"><div><strong>${esc(p.title||p.subject||('Grade '+esc(p.className)+' plan'))}</strong><div class="subtitle">Grade ${esc(p.className)}${p.subject?' • '+esc(p.subject):''}${p.unit?' • Unit: '+esc(p.unit):''}${p.weekOf?' • Week of '+esc(p.weekOf):''}</div></div><div class="item-actions"><select data-plan-status="${esc(p.id)}"><option value="planned"${p.status==='planned'?' selected':''}>${statusLabel.planned}</option><option value="in_progress"${p.status==='in_progress'?' selected':''}>${statusLabel.in_progress}</option><option value="completed"${p.status==='completed'?' selected':''}>${statusLabel.completed}</option></select></div></div>`).join(''):'<p class="subtitle">No lesson plans yet.</p>';
  list.querySelectorAll('[data-plan-status]').forEach(sel=>sel.onchange=async()=>{
   const id=sel.dataset.planStatus, prev=sel.dataset.prevValue||sel.value;
   try{await api(`/api/teacher/lesson-plans/${encodeURIComponent(id)}`,{method:'PATCH',body:JSON.stringify({status:sel.value})}); sel.dataset.prevValue=sel.value; toast('Lesson plan status updated','success');}
...
$('lessonPlanForm')?.addEventListener('submit',async e=>{
 e.preventDefault();
 const statusMsg=$('lessonPlanFormStatus');
 try{
  const objectives=$('lessonPlanObjectives').value.split('\n').map(s=>s.trim()).filter(Boolean);
  const activities=$('lessonPlanActivities').value.split('\n').map(s=>s.trim()).filter(Boolean);
  const materials=$('lessonPlanMaterials').value.split('\n').map(s=>s.trim()).filter(Boolean);
  const body={
   className:$('lessonPlanGrade').value,
   title:$('lessonPlanTitle').value.trim(),
   subject:$('lessonPlanSubject').value.trim(),
   unit:$('lessonPlanUnit').value.trim(),
   weekOf:$('lessonPlanWeek').value,
   courseId:$('lessonPlanCourse').value,
   lessonId:$('lessonPlanLesson').value.trim(),
   assignmentId:$('lessonPlanAssignment').value,
   objectives, activities, materials,
   assessment:$('lessonPlanAssessment').value.trim(),
   status:$('lessonPlanStatus').value,
  };
  const modes=selectedValues('lessonPlanLearningMode'), audiences=selectedValues('lessonPlanAudience');
  if(modes.length){body.learningModes=modes; body.audiences=audiences;}
  await api('/api/teacher/lesson-plans',{method:'POST',body:JSON.stringify(body)});
  if(statusMsg)statusMsg.textContent='✅ Lesson plan saved.';
  e.target.reset();
  toast('Lesson plan saved','success');
  loadTeacherLessonPlans();
 }catch(err){if(statusMsg)statusMsg.textContent='❌ '+err.message; toast(err.message,'error')}
});
const loadBeforeLessonPlans=load;
load=async function(){await loadBeforeLessonPlans(); if($('teacherPanel')?.style.display!=='none'){
 const cs=await api('/api/teacher/courses').catch(()=>({courses:[]}));
 if($('lessonPlanCourse'))$('lessonPlanCourse').innerHTML='<option value="">No course link</option>'+(cs.courses||[]).map(c=>`<option value="${esc(c.id)}">${esc(c.title)} — Grade ${esc(c.className)}</option>`).join('');
 const asg=await api('/api/teacher/assignments').catch(()=>({assignments:[]}));
 if($('lessonPlanAssignment'))$('lessonPlanAssignment').innerHTML='<option value="">No assignment link</option>'+(asg.assignments||[]).map(a=>`<option value="${esc(a.id)}">${esc(a.title)}</option>`).join('');
 loadTeacherLessonPlans();
}};

// V31.108 Lesson Builder/Progress: Practice/Quiz results + self-paced
// completion for Interactive Lesson Builder lessons, read from the small,
// dedicated /api/teacher/lessons/<id>/block-results and
// /api/teacher/courses/<id>/lesson-progress endpoints. Deliberately kept
// separate from Exams/Marklist/Gradebook - no data from those is shown here.
async function loadLessonProgressCourses(){
 const sel=$('lessonProgressCourse'); if(!sel)return;
 const cs=await api('/api/teacher/courses').catch(()=>({courses:[]}));
 sel.innerHTML=(cs.courses||[]).map(c=>`<option value="${esc(c.id)}">${esc(c.title)} — Grade ${esc(c.className)}</option>`).join('');
 if(sel.value) loadLessonProgressForCourse(sel.value);
}
async function loadLessonProgressForCourse(courseId){
 const list=$('lessonProgressList'); if(!list)return;
 if(!courseId){list.innerHTML='Select a course to view lesson progress.';const summary=$('courseProgressSummary');if(summary)summary.innerHTML='';return;}
 list.innerHTML='Loading…';
 loadCourseProgressSummary(courseId);
 try{
  const data=await api(`/api/teacher/courses/${courseId}/lesson-progress`);
  const lessons=data.lessons||[];
  list.innerHTML=lessons.length?lessons.map(l=>`<div class="list-row"><div><strong>${esc(l.title)}</strong><div class="subtitle">${l.status==='published'?'Published':'Draft'}${l.hasQuiz?' • Quiz':''} • Completed ${l.completedCount}/${l.totalStudents} students${l.quizAveragePercentage!=null?` • Quiz avg ${l.quizAveragePercentage}% (${l.quizAttemptCount} attempts)`:''}</div></div>${l.hasQuiz?`<button class="btn btn-outline" type="button" data-lesson-results="${esc(l.id)}">View results</button>`:''}</div>`).join(''):'<p class="subtitle">No lessons in this course yet.</p>';
 }catch(err){list.innerHTML=`<p class="subtitle">❌ ${esc(err.message)}</p>`}
}
// V31.108 Learning Progress & Analytics: course-level summary card
// (Students enrolled/Started/In progress/Completed/Average score),
// alongside the existing per-lesson list above. "Enrolled" is
// eligible-by-grade-and-targeting (the existing BMT access model), not a
// new enrollment record - see teacher_course_progress_summary() in
// lesson_builder_routes.py.
async function loadCourseProgressSummary(courseId){
 const summary=$('courseProgressSummary'); if(!summary)return;
 summary.innerHTML='Loading…';
 try{
  const d=await api(`/api/teacher/courses/${courseId}/progress-summary`);
  const cards=[
   ['Enrolled', d.enrolled??0],
   ['Started', d.started??0],
   ['In progress', d.inProgress??0],
   ['Completed', d.completed??0],
   ['Avg quiz score', d.averageScore==null?'—':`${d.averageScore}%`],
  ];
  summary.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(100px,1fr));gap:8px">`+
   cards.map(([label,value])=>`<div class="list-row" style="text-align:center;display:block"><strong style="font-size:1.1rem">${esc(value)}</strong><div class="subtitle">${esc(label)}</div></div>`).join('')+
   `</div>`;
 }catch(err){summary.innerHTML=`<p class="subtitle">❌ ${esc(err.message)}</p>`}
}
async function loadLessonBlockResults(lessonId){
 const box=$('lessonProgressDetail'); if(!box)return;
 box.innerHTML='Loading…';
 try{
  const data=await api(`/api/teacher/lessons/${lessonId}/block-results`);
  const students=data.students||[];
  box.innerHTML=`<h4 style="margin:0 0 8px">${esc(data.title||'Lesson')} — Practice/Quiz results</h4>`+
   (students.length?`<div style="overflow:auto"><table style="width:100%;min-width:480px"><thead><tr><th>Student</th><th>Block</th><th>Attempts</th><th>Best %</th><th>Last %</th></tr></thead><tbody>`+
    students.flatMap(s=>(s.blocks||[]).map(b=>`<tr><td>${esc(s.name)}</td><td>${esc(b.blockType)}</td><td>${esc(b.attempts)}</td><td>${b.bestPercentage!=null?esc(b.bestPercentage)+'%':'-'}</td><td>${b.lastPercentage!=null?esc(b.lastPercentage)+'%':'-'}</td></tr>`)).join('')+
    `</tbody></table></div>`:'<p class="subtitle">No student attempts yet.</p>');
 }catch(err){box.innerHTML=`<p class="subtitle">❌ ${esc(err.message)}</p>`}
}
document.addEventListener('change',e=>{if(e.target&&e.target.id==='lessonProgressCourse')loadLessonProgressForCourse(e.target.value)});
document.addEventListener('click',e=>{
 if(e.target&&e.target.id==='refreshLessonProgressBtn'){const sel=$('lessonProgressCourse');if(sel)loadLessonProgressForCourse(sel.value);}
 const lid=e.target&&e.target.getAttribute&&e.target.getAttribute('data-lesson-results');
 if(lid)loadLessonBlockResults(lid);
});
const loadBeforeLessonProgress=load;
load=async function(){await loadBeforeLessonProgress(); if($('teacherPanel')?.style.display!=='none'){loadLessonProgressCourses();}};
