...
import { getAuth, onAuthStateChanged, signOut, signInWithEmailAndPassword, createUserWithEmailAndPassword, signInWithCustomToken, RecaptchaVerifier, signInWithPhoneNumber } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js";
...
const app=initializeApp(firebaseConfig),auth=getAuth(app),db=getFirestore(app); let teacher=null; let communityUnsub=null; let communityPollTimer=null; let communitySending=false; let teacherReplyTo=null; let teacherRecordedBlob=null; let teacherRecordedDurationSeconds=null; let teacherRecordingTimer=null; let teacherRecordingStartedAt=0;
...
async function load(){
 try{
  ...
  await loadCourses(); await loadTeacherStudentDirectory(); await loadAiMaterials(); setupTeacherCommunityChat();
...
function renderTeacherCommunity(snapshot){
 ...
 box.innerHTML=rows.length?rows.map(m=>{
  ...
  return `<div class="item-row" style="align-items:flex-start;margin:6px 0"><div class="item-content"><div><b>${esc(label)}</b> <span class="subtitle">${m.createdAt?.toDate?m.createdAt.toDate().toLocaleString():(m.createdAt?new Date(m.createdAt).toLocaleString():'')}</span></div>${m.replyTo?`<div class="subtitle" style="margin:4px 0;padding:5px;border-left:3px solid var(--primary)">↩️ ${esc(m.replyToText||'Original message')}</div>`:''}<div style="margin-top:5px">${esc(m.messageText||m.message||'')}</div>${m.mediaUrl ? (m.mediaType==='image' ? `<img src="${esc(m.mediaUrl)}" alt="Shared image" style="display:block;max-width:240px;max-height:220px;border-radius:12px;margin-top:8px;object-fit:cover;">` : m.mediaType==='audio' ? `<audio controls src="${esc(m.mediaUrl)}" style="display:block;max-width:280px;margin-top:8px;"></audio>${m.durationSeconds ? `<div class="subtitle" style="margin-top:3px;">🎤 ${Math.round(Number(m.durationSeconds))}s voice message</div>` : ''}` : m.mediaType==='video' ? `<video controls src="${esc(m.mediaUrl)}" style="display:block;max-width:280px;max-height:200px;margin-top:8px;border-radius:12px;"></video>` : `<a href="${esc(m.mediaUrl)}" target="_blank" rel="noopener">📎 Open media</a>`) : ''}<div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:7px"><button class="btn btn-neutral" data-t-reply="${m.id}">↩️ Reply</button><button class="btn btn-neutral" data-t-react="${m.id}:heart">❤️ ${reactions.heart||0}</button><button class="btn btn-neutral" data-t-react="${m.id}:like">👍 ${reactions.like||0}</button><button class="btn btn-neutral" data-t-react="${m.id}:dislike">👎 ${reactions.dislike||0}</button><button class="btn btn-neutral" data-t-react="${m.id}:laugh">😂 ${reactions.laugh||0}</button><button class="btn btn-neutral" data-t-react="${m.id}:wow">😮 ${reactions.wow||0}</button><button class="btn btn-neutral" data-t-react="${m.id}:fire">🔥 ${reactions.fire||0}</button><button class="btn btn-neutral" data-t-report="${m.id}">🚩 Report</button></div></div></div>`;
...
onAuthStateChanged(auth,async u=>{
 if(BMT_PREVIEW_ROLE){
  if($('teacherHeader')) $('teacherHeader').style.display='flex';
 ...
 teacher=u;if($('teacherHeader')) $('teacherHeader').style.display=u?'flex':'none';if(window.BMTScannerUI){window.BMT_SCANNER_CONFIG={getToken:()=>teacher?.getIdToken(true)};window.BMTScannerUI.init();}if(!u){if($('teacherLogin')) $('teacherLogin').style.display='block'; if($('teacherSignup')) $('teacherSignup').style.display='block'; if($('teacherDenied')) $('teacherDenied').style.display='none'; if($('teacherPanel')) $('teacherPanel').style.display='none';}else{if($('teacherLogin')) $('teacherLogin').style.display='none'; if($('teacherSignup')) $('teacherSignup').style.display='none'; load();}
});
