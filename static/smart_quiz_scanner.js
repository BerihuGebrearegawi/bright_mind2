...
(function () {
  ...
  async function importQuestions(){
    ...
    try{status('Importing reviewed questions into the existing BMT quiz system…');const d=await api(`/api/scanner/scan/${encodeURIComponent(id)}/import`,'POST',{className:cls,titlePrefix:document.getElementById('smartScanner_title')?.value.trim()||'Scanned Quiz'});status(`Imported ${d.quizCount||0} question(s) into the existing quiz system.`,true);}catch(e){status(e.message||'Import failed.');}
  }
