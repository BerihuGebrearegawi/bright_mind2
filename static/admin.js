...
async function loadTeacherRoster() {
    ...
    try {
        ...
        container.innerHTML=rows.length?rows.map(x=>`<div class="item-row" style="align-items:flex-start"><div class="item-content"><strong>${escapeHtml(x.name||'Teacher')}</strong> (${escapeHtml(x.email||'')})<div class="subtitle">📞 ${escapeHtml(x.phone||'No phone on file')}</div><div class="subtitle">Subjects: ${escapeHtml((x.subjects||[]).join(', ')||'Not set')} | Grades: ${escapeHtml((x.classes||[]).join(', ')||'Not set')}</div><div class="subtitle">${x.status==='active'?'🟢 Active':'🟠 Approved, no activity yet'} · ${x.courseCount||0} courses · ${x.assignmentCount||0} assignments · ${x.studentCount||0} students · Last login: ${escapeHtml(x.lastLoginAt||'Never')}</div></div><div class="item-actions"><button class="btn btn-danger" data-delete-teacher="${escapeHtml(x.uid)}" data-delete-teacher-name="${escapeHtml(x.name||'Teacher')}">🗑️ Delete</button></div></div>`).join(''):'<p class="subtitle">No approved teachers yet.</p>';
