...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (phoneLoginBtn) {
        phoneLoginBtn.addEventListener('click', async () => {
            ...
            if (!phone || !/^\d{6,8}$/.test(pin)) {
                loginError.textContent = 'Please enter a phone number and 6–8 digit PIN.';
                return;
            }
            try {
                showLoadingOverlay('Signing in...');
                const r = await fetch('/api/auth/phone-pin/login', {
                    method: 'POST', headers: {'Content-Type':'application/json'},
                    body: JSON.stringify({phone, pin, role: 'admin'})
                });
                const d = await r.json().catch(() => ({}));
                if (!r.ok || !d.token) throw new Error(d.error || 'Phone sign-in failed.');
                await signInWithCustomToken(auth, d.token);
                hideLoadingOverlay();
            } catch (error) {
                hideLoadingOverlay();
    ...
    try {
        ...
        if (provider === 'google_drive') {
            ...
            const result = provider === 'cloudinary'
                ? (await import('./storage-service.js')).uploadAdminMedia(file, 'raw')
                : (await import('./storage-service.js')).uploadFile(file, 'documents');
            if (!result.success) {
...
window.uploadVideo = async function() {
    ...
    try {
        ...
        if (file) {
            const result = (await import('./storage-service.js')).uploadAdminMedia(file, 'video');
            if (!result.success) {
...
window.uploadQuiz = async function() {
    ...
    try {
        ...
        if (imageFile) {
            // ✅ Image → Cloudinary
            const result = (await import('./storage-service.js')).uploadAdminMedia(imageFile, 'image');
            if (result.success) {
                imageUrl = result.url;
            } else {
...
async function loadAdminChats() {
    ...
    try {
        ...
        container.innerHTML=rows.length?rows.map(chat=>{
            ...
            const media=chat.mediaUrl ? (chat.mediaType==='image' ? `<img src="${escapeHtml(chat.mediaUrl)}" alt="Shared image" style="display:block;max-width:240px;max-height:220px;border-radius:12px;margin-top:8px;object-fit:cover;">` : chat.mediaType==='audio' ? `<audio controls src="${escapeHtml(chat.mediaUrl)}" style="display:block;max-width:280px;margin-top:8px;"></audio>` : chat.mediaType==='video' ? `<video controls src="${escapeHtml(chat.mediaUrl)}" style="display:block;max-width:280px;max-height:200px;margin-top:8px;border-radius:12px;"></video>` : `<a href="${escapeHtml(chat.mediaUrl)}" target="_blank" rel="noopener">📎 Open media</a>`) : ''; return `<div class="item-row" style="align-items:flex-start"><div class="item-content"><div><b>${sender}</b> <span class="subtitle">${escapeHtml(chat.className||'General')}</span></div>${chat.replyTo?`<div class="subtitle" style="margin:4px 0">↩️ Reply to: ${escapeHtml(chat.replyToText||'message')}</div>`:''}<div style="margin-top:5px">${text}</div>${media}<div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:8px"><button class="btn btn-neutral" data-chat-reply="${chat.id}">↩️ Reply</button><button class="btn btn-neutral" data-chat-react="${chat.id}:heart">❤️ ${reactions.heart||0}</button><button class="btn btn-neutral" data-chat-react="${chat.id}:like">👍 ${reactions.like||0}</button><button class="btn btn-neutral" data-chat-react="${chat.id}:dislike">👎 ${reactions.dislike||0}</button><button class="btn btn-neutral" data-chat-react="${chat.id}:laugh">😂 ${reactions.laugh||0}</button><button class="btn btn-neutral" data-chat-react="${chat.id}:wow">😮 ${reactions.wow||0}</button><button class="btn btn-neutral" data-chat-react="${chat.id}:fire">🔥 ${reactions.fire||0}</button></div></div><div class="item-actions"><button class="btn btn-danger" data-chat-delete="${chat.id}">Delete</button></div></div>`;
...
window.saveExamArchive = async function() {
    ...
    try {
        ...
        if (provider === 'gcs' && archiveFile) {
            ...
            const result = (await import('./storage-service.js')).uploadFile(archiveFile, 'documents');
...
async function loadAdminStudentDirectory(){
    ...
    try{ const d=await adminRequest('/api/admin/students'); const rows=d.students||[];
      body.innerHTML=rows.length?rows.map(x=>`<tr><td><b>${ccEsc(x.name)}</b><div class="subtitle">${ccEsc(x.uid)}</div></td><td>${ccEsc(x.phone||'—')}</td><td>${ccEsc(x.email||'—')}</td><td>${ccEsc(x.grade||'—')}</td><td>${x.isPaid?'✅ Paid':'⏳ Unpaid'}</td><td>${ccEsc(x.registeredAt||'—')}</td></tr>`).join(''):'<tr><td colspan="6" class="subtitle">No registered students found.</td></tr>';
      if(status)status.textContent=`${rows.length} registered student${rows.length===1?'':'s'}.`;
    }catch(e){body.innerHTML=`<tr><td colspan="6" class="error-text">${ccEsc(e.message)}</td></tr>`;if(status)status.textContent='';}
}
