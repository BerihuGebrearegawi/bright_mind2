    ...
    try{
      ...
      box.innerHTML=rows.length?rows.map(c=>`
        <div class="card" style="margin-bottom:10px">
          ...
          <div class="subtitle">${esc(c.subject||'')} · Grade ${esc(c.grade||'-')} · Region: ${esc(c.region||'ALL')}${c.zone?' · Zone: '+esc(c.zone):''}${c.woreda?' · Woreda: '+esc(c.woreda):''} · Round ${esc(c.roundNumber)} · ${esc(c.questionCount)} questions · Status: <strong>${esc(c.status)}</strong></div>
          <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">
            ...
            ${c.status==='closed'?`<button class="btn btn-outline" data-policy="${esc(c.id)}">🏅 Award Policy</button><button class="btn btn-primary" data-finalize="${esc(c.id)}">💰 Finalize Awards</button><button class="btn btn-primary" data-winners="${esc(c.id)}">📣 Publish Winners</button>`:''}
          </div>
