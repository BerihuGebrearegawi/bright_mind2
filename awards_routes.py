...
def register_award_routes(app, require_user, firebase_admin_factory):
            ...
            def _epoch(v):
                ...
                if hasattr(v, "timestamp"):
                    try: return float(v.timestamp())
                        ...
                        app.logger.debug("Could not convert challenge timestamp", exc_info=True)
    ...
    def configure_award_policy(challenge_id):
        ...
        try:
            ...
            scholarship_count = max(0, min(MAX_AWARDS, int(body.get("scholarshipCount", 0) or 0)))
            scholarship_label = str(body.get("scholarshipLabel") or "").strip()[:200]
            now = datetime.now(timezone.utc)
            ref.update({"awardCount": award_count, "prizePoolShare": share, "certificateEnabled": certificate_enabled, "prizeCurrency": currency, "scholarshipCount": scholarship_count, "scholarshipLabel": scholarship_label, "awardPolicyUpdatedBy": uid, "awardPolicyUpdatedAt": now, "updatedAt": now})
            return jsonify({"success": True, "challengeId": ref.id, "awardCount": award_count, "prizePoolShare": share, "certificateEnabled": certificate_enabled, "prizeCurrency": currency, "scholarshipCount": scholarship_count, "scholarshipLabel": scholarship_label})
