...
Also create an **unsigned upload preset** named `bright-mind-tutor` in Cloudinary
(Settings → Upload → Upload presets), set `CLOUDINARY_UPLOAD_PRESET` to that name, and make sure
`CLOUDINARY_CLOUD_NAME` matches the `cloudName` in `static/storage-config.js`.
...
`https://<your-app>.onrender.com/api/payments/webhook/chapa` (same secret as `CHAPA_WEBHOOK_SECRET`).
...
| `GEMINI_MODEL` | `gemini-3.6-flash` (the code's default). Older IDs such as `gemini-1.5-flash` / `gemini-2.0-flash` are retired and will fail; check https://ai.google.dev/gemini-api/docs/models for current IDs |
...
## Secrets you generate yourself (declared in `render.yaml`)

Generate each with `python -c "import secrets;print(secrets.token_urlsafe(32))"`.

| Variable | What it's for |
|---|---|
| `ADMIN_PREVIEW_SECRET` | Signs Admin "Dashboard Preview" sessions. Without it the preview feature raises an error |
| `BMT_CERT_VERIFY_SECRET` | Signs/verifies course-certificate verification links |
| `DIAGNOSTIC_KEY` | Protects the diagnostics route |
| `BMT_SCHEDULER_SECRET` | Telegram scheduler cron (see above) |

## Push notifications (optional)

Set `FCM_VAPID_KEY` (Firebase Console → Project settings → Cloud Messaging → Web Push certificates →
Generate key pair) **and** `ENABLE_FCM_PUSH=1`. Leave `ENABLE_FCM_PUSH` at `0` to keep push disabled;
...
2. **Authentication → Settings → Authorized domains:** add your Render domain
   (e.g. `bright-mind-tutor.onrender.com`) and any custom domain. Without this, Google login and
   Phone/reCAPTCHA fail on the deployed site.
3. **Firestore Database:** create it, then deploy rules and indexes (see below).
4. **First admin:** create the admin account in Firebase Authentication, run
   `python setup_admin.py` (needs the service-account JSON), then sign out and sign in again.
...
| `ENABLE_FCM_PUSH` | `0` (set `1` together with `FCM_VAPID_KEY`) |
