    ...
    def student_progress():
        ...
        try:
            ...
            for d in live:
                ...
                if (hasattr(st,'timestamp') and st.timestamp() >= now.timestamp()
                        and x.get('status','scheduled')!='cancelled'
                        and content_visible_or_untargeted(user, x, entitlement)):
                    upcoming.append({'id':d.id,'title':x.get('title','Live class'),'subject':x.get('subject',''),'startAt':iso(st)})
