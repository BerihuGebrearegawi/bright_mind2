...
def register_admin_feature_routes(app, require_admin, firebase_admin_factory, require_user=None):
    ...
    def get_public_org_settings():
        """Public, unauthenticated: just the branding fields dashboards need
        to render the current organization logo. Never returns logoPublicId,
        updatedBy, or any other admin-only bookkeeping field."""
        try:
            snap = db().collection('settings').document('organization').get()
            org = snap.to_dict() if snap.exists else {}
            return jsonify({'success': True, 'logoUrl': (org or {}).get('logoUrl', '')})
        except Exception:
            app.logger.exception('Public organization settings read failed')
            return jsonify({'success': True, 'logoUrl': ''})
    ...
    def save_org_logo():
        ...
        if len(data) > 10 * 1024 * 1024:
            return jsonify({'error': 'Logo is too large. Maximum size is 10 MB.'}), 413
    ...
    def admin_cloudinary_upload():
        ...
        allowed = {
            'image': {'types': {'image/jpeg', 'image/png', 'image/gif', 'image/webp'}, 'max': 10 * 1024 * 1024},
    ...
    def create_admin_book():
        ...
        collection = clean(body.get('collection', 'books'), 40)
        allowed_collections = {'books', 'bookLibrary', 'teacherGuides', 'referenceBooks', 'psychologyBooks'}
        if collection not in allowed_collections:
            return jsonify({'error': 'Unknown library category.'}), 400
        if not title or not class_name:
            return jsonify({'error': 'Book title and class are required.'}), 400
        if provider not in {'gcs', 'google_drive', 'cloudinary', 'external_link'}:
        ...
        if provider == 'external_link':
            ...
            candidate = fields['fileUrl'] or fields['link']
            try:
                parsed = urlparse(candidate)
                if parsed.scheme != 'https' or not parsed.hostname:
                    return jsonify({'error': 'Enter a valid https:// link to the book.'}), 400
            except Exception:
                return jsonify({'error': 'Enter a valid https:// link to the book.'}), 400
            if not fields['link']: fields['link'] = candidate
            if not fields['previewUrl']: fields['previewUrl'] = candidate
            if not fields['downloadUrl']: fields['downloadUrl'] = candidate
        elif provider != 'google_drive' and not (fields['storagePath'] or fields['fileId'] or fields['fileUrl']):
        ...
        try:
            ...
            # V31.108 Phase D: optional Learning Mode / Target Audience tags,
            # same validation as Question Bank/Challenge/Distance Courses.
            # Left unset (the default for this call), a book behaves exactly
            # as before - visible to every grade-matching student regardless
            # of Learning Mode. Only set these to deliberately restrict a
            # specific book (e.g. a Distance-only study guide).
            ...
            if modes: data['learningModes'] = modes
            if audiences: data['audiences'] = audiences
            ref = db().collection(collection).document()
            ref.set(data)
            return jsonify({'success': True, 'id': ref.id, 'collection': collection, 'book': {'id': ref.id, **data}}), 201
        except Exception:
    ...
    def admin_chat_reply():
        ...
        try: duration_seconds=float(duration_seconds) if duration_seconds not in (None,'') else None
        except Exception: duration_seconds=None
        if duration_seconds is not None and (duration_seconds < 0 or duration_seconds > 60):
            return jsonify({'error':'Audio recordings are limited to 60 seconds.'}),400
        ...
        try:
            ...
            ref.set({'senderUid':detail.get('uid'),'senderName':'👑 Admin','userName':'👑 Admin',
                     ...
                     'replyTo':target_id,'replyToText':clean(original.get('messageText') or original.get('message'),300),'durationSeconds':duration_seconds,
    ...
    def community_chat_media_upload():
        ...
        max_bytes = 5*1024*1024 if content_type.startswith('audio/') else (10*1024*1024 if content_type.startswith('image/') else 50*1024*1024)
    ...
    def community_chat_send():
        ...
        if duration_seconds is not None and (duration_seconds < 0 or duration_seconds > 60):
            return jsonify({'error':'Invalid audio duration.'}),400
        try:
            ...
            ref.set({
                ...
                'mediaType':media_type,'profileImage':profile_image,'durationSeconds':duration_seconds,'replyTo':None,
    ...
    def community_chat_reply():
        ...
        try:
            ...
            ref.set({'senderUid':uid,'senderName':name,'userName':name,'role':role,
                     ...
                     'mediaUrl':media_url,'mediaType':media_type,'durationSeconds':duration_seconds,
