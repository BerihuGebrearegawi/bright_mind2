...
class TestCheckAndRecordQuota:
    def test_no_db_getter_always_allows_up_to_the_limit(self, monkeypatch):
        monkeypatch.setenv("AI_FREE_DAILY_LIMIT", "5")
        allowed, remaining = svc.check_and_record_quota(None, "user-1", premium=False)
        assert allowed is True
        assert remaining == 5

    def test_premium_gets_the_higher_limit(self, monkeypatch):
        monkeypatch.setenv("AI_FREE_DAILY_LIMIT", "5")
        monkeypatch.setenv("AI_PREMIUM_DAILY_LIMIT", "40")
        allowed, remaining = svc.check_and_record_quota(None, "user-1", premium=True)
        assert remaining == 40
    ...
    def test_firestore_failure_fails_open(self, monkeypatch):
        """If quota tracking itself breaks, students should not be locked
        out of the tutor entirely - the except-Exception fallback returns
        (True, limit), i.e. fails open rather than closed."""
        monkeypatch.setenv("AI_FREE_DAILY_LIMIT", "5")

        def broken_db_getter():
            raise RuntimeError("Firestore is down")
        allowed, remaining = svc.check_and_record_quota(broken_db_getter, "user-1", premium=False)
        assert allowed is True
        assert remaining == 5
