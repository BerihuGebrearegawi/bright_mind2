...
class TestPhonePinLogin:
    ...
    def test_locked_account_is_rejected_without_checking_pin(self, fake_db, app_module, monkeypatch, fake_firebase_auth):
        """V31.108 audit fix: lockedUntil is now actually read. A locked
        account must be rejected with 429 even when the correct PIN is
        supplied - proving the lock, not just the PIN check, is enforced."""
        import datetime as dt
        _configure_firebase(app_module, monkeypatch)
        fake_firebase_auth.seed_user("s-uid", phone_number=NORMALIZED_PHONE)
        fake_db.seed("users", "s-uid", {"role": "student"})
        future = dt.datetime.now(dt.timezone.utc) + dt.timedelta(minutes=10)
        fake_db.seed("authSecrets", "s-uid", {
            "pinHash": app_module._hash_pin("123456"), "failedAttempts": 10, "lockedUntil": future,
        })
        r = _client(app_module).post("/api/auth/phone-pin/login", json={"phone": VALID_PHONE, "pin": "123456"})
        assert r.status_code == 429

    def test_expired_lock_no_longer_blocks_login(self, fake_db, app_module, monkeypatch, fake_firebase_auth):
        import datetime as dt
        _configure_firebase(app_module, monkeypatch)
        fake_firebase_auth.seed_user("s-uid", phone_number=NORMALIZED_PHONE)
        fake_db.seed("users", "s-uid", {"role": "student"})
        ...
        fake_db.seed("authSecrets", "s-uid", {
            "pinHash": app_module._hash_pin("123456"), "failedAttempts": 10, "lockedUntil": past,
        })
        r = _client(app_module).post("/api/auth/phone-pin/login", json={"phone": VALID_PHONE, "pin": "123456"})
        assert r.status_code == 200

    def test_tenth_wrong_pin_locks_the_account(self, fake_db, app_module, monkeypatch, fake_firebase_auth):
        _configure_firebase(app_module, monkeypatch)
        fake_firebase_auth.seed_user("s-uid", phone_number=NORMALIZED_PHONE)
        fake_db.seed("users", "s-uid", {"role": "student"})
        fake_db.seed("authSecrets", "s-uid", {"pinHash": app_module._hash_pin("123456"), "failedAttempts": 9})
        r = _client(app_module).post("/api/auth/phone-pin/login", json={"phone": VALID_PHONE, "pin": "000000"})
        assert r.status_code == 401
        assert fake_db.dump("authSecrets")["s-uid"]["lockedUntil"] is not None
        # And the very next attempt is now blocked by the lock, even with
        # the right PIN.
        r2 = _client(app_module).post("/api/auth/phone-pin/login", json={"phone": VALID_PHONE, "pin": "123456"})
        assert r2.status_code == 429


class TestPhonePinReset:
    def test_signed_out_is_rejected(self, fake_db, app_module, monkeypatch):
        require_user_denied(app_module, monkeypatch)
        r = _client(app_module).post("/api/auth/phone-pin/reset", json={"pin": "123456"})
        assert r.status_code == 401

    def test_missing_phone_number_claim_is_rejected(self, fake_db, app_module, monkeypatch):
        require_user_as(app_module, monkeypatch, uid="s-uid")
        r = _client(app_module).post("/api/auth/phone-pin/reset", json={"pin": "123456"})
        assert r.status_code == 400

    def test_non_phone_sign_in_provider_is_rejected(self, fake_db, app_module, monkeypatch):
        """V31.108 audit fix: a phone_number claim alone doesn't prove this
        token came from a fresh OTP verification - firebase.sign_in_provider
        must say 'phone' too."""
        require_user_as(app_module, monkeypatch, uid="s-uid", phone_number=NORMALIZED_PHONE,
                         firebase={"sign_in_provider": "custom"})
        fake_db.seed("users", "s-uid", {"role": "student"})
        r = _client(app_module).post("/api/auth/phone-pin/reset", json={"pin": "123456"})
        assert r.status_code == 400

    def test_phone_verified_reset_succeeds(self, fake_db, app_module, monkeypatch):
        require_user_as(app_module, monkeypatch, uid="s-uid", phone_number=NORMALIZED_PHONE,
                         firebase={"sign_in_provider": "phone"})
        fake_db.seed("users", "s-uid", {"role": "student"})
        r = _client(app_module).post("/api/auth/phone-pin/reset", json={"pin": "123456"})
        assert r.status_code == 200
        secret = fake_db.dump("authSecrets")["s-uid"]
        assert secret["failedAttempts"] == 0
        assert secret["lockedUntil"] is None
