...
def register_award_ledger_routes(app, require_user, firebase_admin_factory):
    ...
    def finalize_awards(challenge_id):
        ...
        try:
            ...
            scholarship_count = max(0, min(MAX_AWARDS, int(c.get("scholarshipCount", 0) or 0)))
            scholarship_label = str(c.get("scholarshipLabel") or "").strip()
            now = datetime.now(timezone.utc)
            ...
            for rank, (winner, ref, existing) in enumerate(zip(winners, award_refs, existing_award_snaps), 1):
                ...
                transaction.set(ref, {
                    ...
                    "isScholarship": rank <= scholarship_count,
                    "scholarshipLabel": scholarship_label if rank <= scholarship_count else "",
