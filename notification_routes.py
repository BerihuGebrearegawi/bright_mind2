...
def register_notification_routes(app, require_user, require_admin, firebase_admin_factory, create_notification):
    ...
    def targeted_announcement():
        """Admin announcement to all students of a grade (optionally further
        narrowed by Learning Mode / Audience) or a single user.

        V31.108 Phase F1: learningModes/audiences are optional, independent
        filters on top of the existing grade match - the allowed-value sets
        match exactly what admin_features_routes.py already validates for
        Materials, and the audience computation itself reuses
        targeting_access.student_audience_set(), so "Scholarship"/
        "Competition"/Free-vs-Paid-Regular mean the same thing here as they
        do for a book or video. Left unset (the default, matching every
        announcement created before this change), grade targeting behaves
        exactly as before - every matched student receives it regardless of
        Learning Mode/Audience. A single targetUid announcement is
        unaffected: an admin who names one exact recipient has already
        disambiguated who it's for.
        """
        ...
        supplied_audiences = isinstance(body.get('audiences'), list) and bool(body.get('audiences'))
        audiences = list(dict.fromkeys(x for x in audiences if x in allowed_audiences))
        if supplied_audiences and not audiences and not target_uid:
            return jsonify({'error': 'No valid audience supplied.'}), 400
        ...
        try:
            ...
            if target_uid:
                ...
                # Each filter is independently optional. Deliberately NOT
                # using targeting_access.content_target_allowed()/
                # content_visible_or_untargeted() here: those default a
                # missing `audiences` list to ["Free Regular","Paid
                # Regular"], which is correct for tagged CONTENT records
                # but wrong here - it would silently exclude every Distance
                # student from a learningModes-only ["Distance"] filter,
                # since Distance students never carry a Free/Paid Regular
                # audience. Confirmed by test_targeted_announcement.py
                # (test_distance_only_excludes_regular_students failed
                # under the content_target_allowed-based version).
                def _matches_targeting(user, entitlement):
                    if modes:
                        user_mode = str(user.get('learningMode') or 'Regular').strip()
                        if user_mode not in modes:
                            return False
                    if audiences and not (student_audience_set(user, entitlement) & set(audiences)):
                        return False
                    return True
                ...
                for uid, snap in matched.items():
                    if modes or audiences:
                        user = snap.to_dict() or {}
                        try:
                            entitlement = database.collection('entitlements').document(uid).get().to_dict() or {}
                        except Exception:
                            entitlement = {}
                        if not _matches_targeting(user, entitlement):
                            continue
