...
checks = [
    ('_require_user_bearer()' in app and 'Admin or approved teacher access required for document uploads.' in app, 'document upload is role-gated'),
    ...
    ("@app.post('/api/storage/document')" in app and "from cloudinary_storage import upload_document, signed_url" in app, 'server exposes controlled document upload'),
    ("@app.post('/api/storage/document-url')" in app and "signed_url(path)" in app, 'server can refresh short-lived signed document URLs'),
    ('cloudinary==1.45.0' in requirements, 'Cloudinary SDK pinned'),
