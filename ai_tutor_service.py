...
def retrieve_material_context(db_getter, grade, subject, question, limit=6, uid=None):
    """Return relevant active BMT chunks with source metadata.

    V31.108 Materials Targeting: when `uid` is supplied, each candidate
    chunk is additionally checked with targeting_access.
    content_visible_or_untargeted() - the same helper and the same
    Learning Mode / Audience model already enforced for the Digital
    Library (Books/Videos), so a chunk from a Distance-only or
    Scholarship-only teacher material cannot be pulled into another
    student's AI Tutor answer. A chunk with no learningModes/audiences set
    (every chunk created before this change, and any new one left
    untargeted) stays visible to everyone, exactly as before - `uid` is
    additive, never a new restriction on existing untargeted content.
    """
    if not db_getter:
        return []
    try:
    ...
    for snap in docs:
        ...
        if uid:
            ...
            if not content_visible_or_untargeted(user, data, entitlement):
                continue
...
def check_and_record_quota(db_getter: Optional[Callable[[], Any]], uid: str, premium: bool):
    ...
    if not db_getter:
        raise RuntimeError("AI usage quota service is unavailable. Please try again shortly.")
    try:
        ...
        # Never bypass the daily quota when Firestore is unavailable or the
        # usage transaction fails.  A fail-open quota turns an infrastructure
        # problem into unlimited paid AI usage.
        raise RuntimeError("AI usage quota service is temporarily unavailable. Please try again shortly.") from exc
...
def _gemini_headers(api_key):
    """Pin the Interactions API response shape instead of floating on
    whatever Google defaults to. Google's May 2026 breaking-change round for
    this endpoint moved the response from a flat `outputs`/`output_text`
    shape to a `steps` timeline (see
    https://ai.google.dev/gemini-api/docs/interactions-breaking-changes-may-2026);
    _extract_interaction_text() already handles both shapes defensively, but
    sending Api-Revision explicitly means a future Google-side default change
    can't silently swap which shape we get back without us noticing."""
    return {
