...
BMT_VERSION = os.getenv("BMT_VERSION", "31.108").strip() or "31.108"
...
# Firebase Admin SDK initialization must be serialized because Flask handles
# requests concurrently. Without a lock, two simultaneous API requests can
# both observe an empty firebase_admin._apps and race into initialize_app(),
# producing the intermittent "default Firebase app already exists" error.
_FIREBASE_INIT_LOCK = threading.RLock()

def _client_key():
    # On Render, traffic reaches the app through Cloudflare/Render proxies.
    # Prefer Cloudflare's client-IP header because it is overwritten by the
    # proxy, so a caller cannot rotate it to evade the limiter. X-Forwarded-For
    # is only accepted when the request also carries Render/Cloudflare's
    # proxy marker; otherwise a direct caller can spoof that header.
...
def _security_headers(response):
    ...
    if request.path.startswith("/api/") or request.path in {"/healthz", "/readyz"}:
        ...
        if request.args.get("v"):
            # Has a ?v=X.XX cache-busting query string - a changed file gets a
            # new URL, so it's safe to cache indefinitely without revalidation.
            response.headers.setdefault("Cache-Control", "public, max-age=31536000, immutable")
        else:
            # No version param (e.g. logo/icon files) - cache for a day so
            # repeat page loads are fast, but an updated logo is still picked
            # up reasonably quickly instead of being stuck for a year.
...
# V31.107 — secure admin dashboard preview tickets.
# A preview ticket is a short-lived, single-use, HMAC-signed pointer at one
# real (role, uid) pair. It never grants access by itself: the target page
# must exchange it (once) at /api/admin/preview-signin for a genuine Firebase
# custom token for that exact uid, so the preview shows the real dashboard's
# real data instead of fabricated placeholder content.
def _preview_secret():
    return (os.getenv('ADMIN_PREVIEW_SECRET') or os.getenv('SECRET_KEY') or '').strip()

_PREVIEW_USED_NONCES = {}
_PREVIEW_NONCE_LOCK = threading.RLock()

def _preview_nonce_consume(nonce, exp):
    # Single-use enforcement so a preview ticket copied out of a URL (browser
    # history, referrer, server logs) cannot be replayed after first use.
    # The store is process-local and self-prunes, matching this file's
    # existing in-memory rate-limiter pattern.
    now = int(time.time())
    with _PREVIEW_NONCE_LOCK:
        for key, stored_exp in list(_PREVIEW_USED_NONCES.items()):
            if stored_exp < now:
                _PREVIEW_USED_NONCES.pop(key, None)
        if nonce in _PREVIEW_USED_NONCES:
            return False
        _PREVIEW_USED_NONCES[nonce] = exp
        return True

def _make_preview_token(role, uid, admin_uid, ttl_seconds=300):
    secret = _preview_secret()
    if not secret:
        raise RuntimeError('ADMIN_PREVIEW_SECRET is not configured.')
    import secrets as _secrets
    payload = {
        'role': role, 'uid': uid, 'adminUid': admin_uid,
        'exp': int(time.time()) + int(ttl_seconds), 'nonce': _secrets.token_hex(8),
    }
...
def _verify_preview_token(token, role, uid, consume=True):
    secret = _preview_secret()
    if not secret or not token or not role or not uid:
        return None
    try:
        ...
        if payload.get('role') != role or payload.get('uid') != uid:
            return None
        if int(payload.get('exp', 0)) < int(time.time()):
            return None
        if consume and not _preview_nonce_consume(payload.get('nonce', ''), int(payload.get('exp', 0))):
            return None
        return payload
    except Exception:
        return None
...
def admin_preview_token():
    ...
    role = str(body_json.get('role') or '').strip().lower()
    uid = str(body_json.get('uid') or '').strip()
    if role not in {'student', 'teacher', 'parent'}:
        return jsonify({'error': 'Invalid preview role.'}), 400
    if not uid:
        return jsonify({'error': 'Select a real account to preview.'}), 400
    try:
        ...
        actual_role = str(data.get('role') or data.get('accountType') or '').strip().lower()
        role_ok = actual_role == role or (role == 'student' and actual_role in {'', 'learner'})
        if not role_ok:
            return jsonify({'error': 'That account does not match the selected role.'}), 400
        admin_uid = str(detail.get('uid') or detail.get('user_id') or detail.get('sub') or '')
        token = _make_preview_token(role, uid, admin_uid)
        return jsonify({'success': True, 'role': role, 'uid': uid, 'token': token, 'expiresIn': 300}), 200
    except RuntimeError as exc:
        return jsonify({'error': str(exc), 'requestId': getattr(g, 'request_id', '')}), 503
    except Exception:
        app.logger.exception('Preview token issuance failed')
        return jsonify({'error': 'Could not create the preview link.'}), 500

@app.get('/api/admin/preview-signin')
def admin_preview_signin():
    """Exchanges a single-use preview ticket for a real, short-lived Firebase
    sign-in token for the exact target account, so the dashboard the admin
    lands on loads that account's genuine data through the same code path a
    real user would use. Every exchange is written to adminAuditLogs."""
    role = str(request.args.get('role') or '').strip().lower()
    uid = str(request.args.get('uid') or '').strip()
    token = str(request.args.get('token') or '').strip()
    if role not in {'student', 'teacher', 'parent'} or not uid:
        return jsonify({'error': 'Invalid preview link.'}), 400
    payload = _verify_preview_token(token, role, uid)
    if not payload:
        return jsonify({'error': 'This preview link is invalid, expired, or already used.'}), 401
    fb = _firebase_admin_from_env()
    if not fb:
        return jsonify({'error': 'Server Firebase credentials are not configured.'}), 503
    try:
        from firebase_admin import auth as firebase_auth, firestore
        custom_token = firebase_auth.create_custom_token(uid, {'previewSession': True, 'previewRole': role})
        ...
        try:
            ...
            db.collection('adminAuditLogs').document().set({
                'action': 'admin_dashboard_preview',
                'target': uid,
                'adminUid': payload.get('adminUid', ''),
                'previewRole': role,
                'createdAt': datetime.now(timezone.utc),
            })
        except Exception:
            app.logger.exception('Preview audit log write failed for uid=%s', uid)
            return jsonify({'error': 'Could not record the preview audit event. Please try again.'}), 503
        name = profile.get('displayName') or profile.get('name') or profile.get('fullName') or ''
        return jsonify({'success': True, 'token': custom_token, 'uid': uid, 'role': role, 'name': name}), 200
    except Exception:
        app.logger.exception('Preview sign-in exchange failed')
        return jsonify({'error': 'Could not start the preview session.'}), 500
...
@app.route('/auth')
def auth_page():
    """V31.94 unified email-first authentication entry point: email -> role
    selection (new accounts) -> tailored registration -> dashboard redirect.
    Additive - existing per-dashboard login forms are untouched and keep
    working; this is the new recommended front door linked from index.html."""
...
def _can_access_gcs_book(db, path, detail):
    ...
    for snap in matches:
        ...
        # V31.108 Phase D: a book an admin has explicitly restricted to a
        # Learning Mode/Audience must be blocked here too, not only hidden
        # from the /api/library listing - a signed URL is a bearer
        # credential and this is the last server-side check before minting
        # one. Untagged (legacy) books are unaffected.
...
def document_access_url():
    """Mint a freshly-signed Cloudinary URL only for an authorized book.

    The URL is re-signed on every call after re-checking authorization via
    _can_access_gcs_book - see cloudinary_storage.py's module docstring for
...
def auth_check_email():
    """Tell the sign-in page whether an email already has an account.

    V31.83 fix: the client used Firebase's fetchSignInMethodsForEmail() to
    decide sign-in vs. registration. Firebase's default "Email Enumeration
    Protection" (enabled on this project) makes that client call always
    return an empty list, so every visitor -- including existing students,
    teachers, parents, and the admin -- was routed to the registration step
    instead of sign-in. New registration then failed with "email-already-in-use"
    for anyone who already had an account, which is what looked like the
    dashboards/admin "getting mixed up" at login.
    Checking existence with the Admin SDK server-side is unaffected by that
    client-side protection and is the officially recommended workaround.
    """
    body = request.get_json(silent=True) or {}
    ...
    try:
        # V31.108 fix: every other Firebase-touching route calls
        # _firebase_admin_from_env() first so the Admin SDK's default app is
        # guaranteed to be initialized. This route skipped that step and
        # called firebase_admin.auth directly, so on a fresh worker (no
        # prior Firebase call yet) it raised "The default Firebase app does
        # not exist" -- caught by the generic except below and returned as
        # a 502. The client's email step then failed before it could route
        # a returning student to quick sign-in, dropping everyone into the
        # slower registration path instead.
        fb = _firebase_admin_from_env()
        if fb is None:
            return jsonify({'error': 'Sign-in is not configured on the server.', 'requestId': getattr(g, 'request_id', '')}), 503
        from firebase_admin import auth as firebase_auth
        try:
            user = firebase_auth.get_user_by_email(email)
            # V31.108 fix: tell the client which sign-in methods this account
            # actually has. Without this, an account created only via Google
            # (no password) was routed into the password sign-in step, where
            # Firebase's generic "wrong password" error left the user stuck
            # with no way to know they should use "Continue with Google"
            # instead -- functionally a dead end / apparent duplicate-account
            # conflict.
            providers = sorted({p.provider_id for p in (user.provider_data or [])})
            return jsonify({'exists': True, 'providers': providers})
        except firebase_auth.UserNotFoundError:
            return jsonify({'exists': False})
    except RuntimeError as exc:
        return jsonify({'error': str(exc), 'requestId': getattr(g, 'request_id', '')}), 503
    except Exception:
        app.logger.exception('Email existence check failed')
        return jsonify({'error': 'Could not check that email right now. Please try again.', 'requestId': getattr(g, 'request_id', '')}), 502
...
@app.route('/healthz')
def healthz():
    return {"status": "ok", "version": BMT_VERSION, "requestId": getattr(g, "request_id", "")}, 200
...
def readyz():
    ...
    # Chapa is the currently documented/deployed gateway (see DEPLOYMENT.md);
    # Telebirr/CBE are supported by the same generic webhook route but are
    # not part of the standard deployment. A readiness check that only
    # looked at Telebirr/CBE would report "not_ready" forever on a
    # Chapa-only deployment set up exactly per DEPLOYMENT.md.
    chapa_configured = bool(os.getenv("CHAPA_SECRET_KEY", "").strip()) and bool(os.getenv("CHAPA_WEBHOOK_SECRET", "").strip())
    ...
    payment_webhook_configured = chapa_configured or telebirr_webhook_configured or cbe_webhook_configured
...
def admin_config_status():
    ...
    checks = {
        ...
        # Chapa is the documented/deployed gateway (DEPLOYMENT.md); Telebirr/CBE
        # share the same generic webhook route but are not part of the
        # standard deployment - see the readyz check above for the same split.
        "chapaConfigured": bool(os.getenv("CHAPA_SECRET_KEY", "").strip() and os.getenv("CHAPA_WEBHOOK_SECRET", "").strip()),
        ...
        # Matches the credentials cloudinary_storage.py actually requires
        # (DEPLOYMENT.md's "File storage" section) - CLOUDINARY_UPLOAD_PRESET
        # is a separate, optional fallback used only by the org-logo unsigned
        # upload path in admin_features_routes.py, and was never required for
        # document/book/AI-material/quiz-scan storage. Checking for it here
        # made this diagnostic report "cloudinary: false" on deployments that
        # were actually fully configured.
        "cloudinary": bool(os.getenv("CLOUDINARY_CLOUD_NAME", "").strip() and os.getenv("CLOUDINARY_API_KEY", "").strip() and os.getenv("CLOUDINARY_API_SECRET", "").strip()),
        "cloudinaryUnsignedUploadPreset": bool(os.getenv("CLOUDINARY_UPLOAD_PRESET", "").strip()),
    }
...
def library_list():
    ...
    try:
        ...
        # V31.108 Phase D: fetch this student once so untagged legacy items
        # stay visible to everyone (see content_visible_or_untargeted) while
        # any book an admin has explicitly targeted is respected.
        uid = str(detail.get('uid') or '').strip()
        is_admin = detail.get('admin') is True or detail.get('role') == 'admin'
        user = db.collection('users').document(uid).get().to_dict() or {} if uid else {}
        # V31.108 audit fix: the Teacher Dashboard's Digital Library panel calls
        # this endpoint too. An approved teacher has no student grade/Learning
        # Mode on file, so the student-only gate below returned an empty
        # library for every teacher. Approved teachers are treated like admins
        # here (same approval rule as teacher_routes._teacher); students are
        # unaffected.
        is_staff = is_admin
        if not is_staff and uid:
            try:
                t_snap = db.collection('teachers').document(uid).get()
                t_data = (t_snap.to_dict() or {}) if t_snap.exists else {}
                is_staff = bool(t_data.get('approved') or t_data.get('status') == 'approved')
            except Exception:
                is_staff = False
        if not is_staff:
            own_grade = str(user.get('class') or user.get('grade') or '').strip()
            if not own_grade:
                return jsonify({'items': []}), 200
            if grade and grade != own_grade:
                return jsonify({'items': []}), 200
            grade = own_grade
        try:
            entitlement = db.collection('entitlements').document(uid).get().to_dict() or {} if uid else {}
        except Exception:
            entitlement = {}
        for collection_name, default_category in sources:
            for snap in db.collection(collection_name).limit(250).stream():
                x=snap.to_dict() or {}
                if not is_staff and not content_visible_or_untargeted(user, x, entitlement):
                    continue
                ...
                seen.add(key)
                rows.append({'id':snap.id,'sourceCollection':collection_name,'title':title,'grade':g,'subject':sub,'category':c,'categoryLabel':labels.get(c,c.replace('_',' ').title()),'description':x.get('description') or '','url':x.get('previewUrl') or x.get('link') or x.get('fileUrl') or x.get('downloadUrl') or '',
                    # V31.108: additive fields only, so the /booksSection UI
                    # (loadStudentContent in student.js) can migrate off its
                    # direct Firestore `books` read onto this endpoint
                    # without losing the cover image or the separate
                    # preview/download links. Does not touch the
                    # authorization/filtering above in any way.
                    'previewUrl':x.get('previewUrl') or x.get('link') or '','downloadUrl':x.get('downloadUrl') or x.get('fileUrl') or '','coverUrl':x.get('coverUrl') or x.get('thumbnailUrl') or x.get('imageUrl') or '',
                    'storageProvider':x.get('storageProvider') or '','storagePath':x.get('storagePath') or x.get('fileId') or ''})
...
def ai_translate():
    ...
    try:
        r=requests.post(GEMINI_API_URL.format(model=model)+f'?key={api_key}',json={'contents':[{'parts':[{'text':prompt}]}],'generationConfig':{'maxOutputTokens':4096}},timeout=45)
    ...
    try:
        ...
        if db_getter:
            try:
                ...
                premium = _active_premium_entitlement(ent)
            except Exception:
                premium = False
        ...
        materials = retrieve_material_context(db_getter, grade, subject, message, uid=uid)
...
def ai_homework_coach():
    ...
    try:
        ...
        materials = retrieve_material_context(db_getter, grade, subject, question, limit=6, uid=uid)
        prompt = (
...
def ai_practice_quiz():
    ...
    try:
        ...
        if not allowed:
            return jsonify({"error": "Daily AI Tutor limit reached.", "remaining": 0}), 429
        materials = retrieve_material_context(db_getter, grade, subject, message, limit=6, uid=uid)
        if not materials:
...
def _firebase_admin_from_env():
    ...
    try:
        ...
        # Flask's threaded server may enter this function concurrently from
        # several dashboard/API requests. Keep the check and initialization
        # in one critical section so only one request can create the default
        # Firebase app. If another caller has already initialized it, reuse it.
        with _FIREBASE_INIT_LOCK:
            if not firebase_admin._apps:
                firebase_admin.initialize_app(credentials.Certificate(parsed))
            else:
                # Accessing the default app verifies that the app we are about
                # to reuse is actually present and avoids relying solely on the
                # private _apps mapping for the successful path.
                try:
...
def _active_premium_entitlement(entitlement):
    """Return True only for a server-issued paid entitlement with a valid expiry."""
    entitlement = entitlement or {}
    if not bool(entitlement.get("premium")):
        return False
...
def _normalize_phone(phone):
    """Normalize Ethiopian/local phone numbers to Firebase E.164 format."""
    ...
    if value.startswith("+251"):
        ...
        local = value
    if not re.fullmatch(r"[79]\d{8}", local):
        return None
    return "+251" + local
...
def _verify_pin(pin, encoded):
    try:
        algorithm, n, r, p, salt_b64, digest_b64 = str(encoded or "").split("$", 5)
        if algorithm != "scrypt":
            return False
        pad = lambda x: x + "=" * (-len(x) % 4)
        salt = base64.urlsafe_b64decode(pad(salt_b64))
        expected = base64.urlsafe_b64decode(pad(digest_b64))
        actual = hashlib.scrypt(pin.encode("utf-8"), salt=salt, n=int(n), r=int(r), p=int(p), dklen=len(expected))
        return hmac.compare_digest(actual, expected)
    except Exception:
        return False


@app.post('/api/auth/phone-pin/register')
def phone_pin_register():
    """Create a BMT account using phone + PIN without exposing the PIN to Firebase."""
    body = request.get_json(silent=True) or {}
    role = str(body.get('role') or '').strip().lower()
    name = str(body.get('name') or '').strip()[:120]
    phone = _normalize_phone(body.get('phone'))
    pin = str(body.get('pin') or '').strip()
    class_name = str(body.get('className') or '').strip()[:40]
    region = str(body.get('region') or '').strip()[:60]
    if role not in {'student', 'teacher', 'parent'}:
        return jsonify({'error': 'Choose Student, Teacher, or Parent.'}), 400
    if not name:
        return jsonify({'error': 'Full name is required.'}), 400
    if not phone:
        return jsonify({'error': 'Enter a valid Ethiopian phone number.'}), 400
    if not re.fullmatch(r'\d{6,8}', pin):
        return jsonify({'error': 'PIN must contain 6 to 8 digits.'}), 400
    if role == 'student' and not class_name:
        return jsonify({'error': 'Class is required for a student account.'}), 400
    fb = _firebase_admin_from_env()
    if not fb:
        return _auth_error('Server Firebase credentials are not configured.', 503)
    try:
        from firebase_admin import auth as firebase_auth, firestore
        db = firestore.client()
        try:
            ...
            pass
        user = firebase_auth.create_user(phone_number=phone, display_name=name)
        now = datetime.now(timezone.utc)
        profile = {
            'uid': user.uid, 'name': name, 'phone': phone, 'phoneVerified': False,
            'class': class_name, 'region': region, 'role': role, 'accountType': role,
            'email': '', 'registeredAt': now, 'updatedAt': now,
            'progress': {'videos': {}, 'quizzes': {}}, 'payments': [],
            'isAdmin': False, 'profileImage': '', 'bio': '', 'isPaid': False,
            # V31.108 Phase B: every student gets an explicit learningMode at
            # signup so targeting_access.py has a real value to read instead
            # of silently defaulting. Admin can change it later via
            # POST /api/admin/students/<uid>/learning-mode.
            'learningMode': 'Regular',
        }
        db.collection('users').document(user.uid).set(profile, merge=True)
        db.collection('authSecrets').document(user.uid).set({'pinHash': _hash_pin(pin), 'updatedAt': now}, merge=True)
        if role == 'teacher':
            db.collection('teacherRequests').document().set({
                'uid': user.uid, 'phone': phone, 'email': '', 'name': name,
                'bio': '', 'educationLevel': '', 'institution': '', 'experience': '',
                'experienceYears': '0', 'certifications': '', 'subjects': [], 'classes': [],
                'status': 'pending', 'createdAt': now, 'updatedAt': now,
                'source': 'phone-pin'
            })
        return jsonify({'success': True, 'uid': user.uid, 'role': role, 'phone': phone, 'phoneVerified': False}), 201
    except Exception as exc:
        app.logger.exception('Phone/PIN registration failed')
        return jsonify({'error': 'Could not create the account. Please try again.'}), 500


@app.post('/api/auth/phone-pin/login')
def phone_pin_login():
    body = request.get_json(silent=True) or {}
    phone = _normalize_phone(body.get('phone'))
    pin = str(body.get('pin') or '').strip()
    requested_role = str(body.get('role') or '').strip().lower()
    if not phone or not re.fullmatch(r'\d{6,8}', pin):
        return jsonify({'error': 'Enter your phone number and 6–8 digit PIN.'}), 400
    if requested_role and requested_role not in {'student', 'teacher', 'parent'}:
    ...
    try:
        ...
        try:
            ...
            return jsonify({'error': 'No BMT account was found for this phone number.'}), 401
        snap = db.collection('users').document(user.uid).get()
        data = snap.to_dict() or {}
        role = str(data.get('role') or data.get('accountType') or '').lower()
        is_admin = data.get('isAdmin') is True or role == 'admin'
        if is_admin:
            # Admin accounts are intentionally excluded from phone+PIN sign-in.
            # A PIN is a much smaller credential space than email + password,
            # and this endpoint must never become an alternate path to an
            # admin-privileged session. Verified before the PIN is even
            # checked, so this endpoint can never be used as a PIN-guessing
            # oracle against an administrator account.
            return jsonify({'error': 'Admin accounts must sign in with email and password.'}), 403
        if role not in {'student', 'teacher', 'parent'}:
            return jsonify({'error': 'This account has no valid BMT role.'}), 403
        if requested_role and role != requested_role:
            return jsonify({'error': f'This phone number belongs to a {role} account.'}), 403
        if data.get('isSuspended'):
            reason = str(data.get('suspendReason') or '').strip()
            message = 'Your account has been suspended.' + (f' Reason: {reason}.' if reason else '') + ' Contact support if you believe this is a mistake.'
            return jsonify({'error': message}), 403
        secret_ref = db.collection('authSecrets').document(user.uid)
        now = datetime.now(timezone.utc)
        # Failed-attempt counting must be atomic. A read/modify/write sequence
        # lets concurrent wrong-PIN requests overwrite each other's counters
        # and can therefore bypass the lockout threshold. Firestore retries the
        # transaction when another request changes this document concurrently.
        # V31.108 audit fix: `now` was never defined here (NameError -> HTTP 500
        # for every phone+PIN sign-in), and the transaction was driven by hand
        # (transaction.get()/commit() without begin), which the real Firestore
        # SDK rejects. @firestore.transactional begins, retries and commits it.
        # V31.108 audit fix: lockedUntil was written on the 10th failed
        # attempt but never read back, so the lockout never actually
        # engaged - a wrong PIN could be retried indefinitely. Checked
        # first, inside the same transaction, so a PIN guess made while
        # locked never even reaches _verify_pin.
        @firestore.transactional
        def _verify_and_count(transaction):
            secret_snap = secret_ref.get(transaction=transaction)
            secret_data = secret_snap.to_dict() or {}
            locked_until = secret_data.get('lockedUntil')
            if locked_until and locked_until > now:
                return 'locked'
            if not _verify_pin(pin, secret_data.get('pinHash')):
                failures = int(secret_data.get('failedAttempts', 0) or 0) + 1
                update = {'failedAttempts': failures, 'updatedAt': now}
                if failures >= 10:
                    update['lockedUntil'] = now + timedelta(minutes=15)
                transaction.set(secret_ref, update, merge=True)
                return 'invalid'
            transaction.set(secret_ref, {'failedAttempts': 0, 'lockedUntil': None, 'updatedAt': now}, merge=True)
            return 'ok'
        result = _verify_and_count(db.transaction())
        if result == 'locked':
            return jsonify({'error': 'Too many incorrect attempts. Please try again in a few minutes.'}), 429
        if result != 'ok':
            return jsonify({'error': 'Invalid phone number or PIN.'}), 401
        claims = {'role': role, 'phoneLogin': True}
        if role == 'admin':
            claims['admin'] = True
        custom_token = firebase_auth.create_custom_token(user.uid, claims)
        if isinstance(custom_token, bytes):
            custom_token = custom_token.decode('utf-8')
        return jsonify({'success': True, 'token': custom_token, 'uid': user.uid, 'role': role, 'phoneVerified': bool(data.get('phoneVerified'))}), 200
    except Exception:
        app.logger.exception('Phone/PIN login failed')
        return jsonify({'error': 'Could not complete phone sign-in. Please try again.'}), 500


@app.post('/api/auth/admin-token')
def auth_admin_token():
    """Upgrade a valid Firebase session to admin claims when the server-side
    BMT profile explicitly marks the account as an administrator.

    This supports legacy admin accounts whose Firebase custom claims were never
    initialized, while keeping the authorization decision server-side.
    """
    ok, detail = _require_user_bearer()
    if not ok:
        return detail
    try:
        ...
        snap = db.collection('users').document(uid).get()
        data = snap.to_dict() or {}
        role = str(data.get('role') or data.get('accountType') or '').strip().lower()
        if data.get('isAdmin') is not True and role != 'admin':
            return jsonify({'error': 'This account is not authorized as an administrator.'}), 403
        token = firebase_auth.create_custom_token(uid, {'admin': True, 'role': 'admin'})
        if isinstance(token, bytes):
            token = token.decode('utf-8')
        return jsonify({'success': True, 'token': token, 'uid': uid, 'role': 'admin'}), 200
    except Exception:
        app.logger.exception('Admin token upgrade failed')
        return jsonify({'error': 'Could not verify administrator authorization.'}), 500


@app.post('/api/auth/phone-pin/mark-verified')
def phone_pin_mark_verified():
    ok, detail = _require_user_bearer()
    if not ok:
        return detail
    try:
        ...
        db = firestore.client()
        uid = str(detail.get('uid') or '').strip()
        phone = str(detail.get('phone_number') or '').strip()
        if not uid or not phone:
            return jsonify({'error': 'Phone verification could not be confirmed.'}), 400
        db.collection('users').document(uid).set({'phone': phone, 'phoneVerified': True, 'updatedAt': datetime.now(timezone.utc)}, merge=True)
        return jsonify({'success': True, 'phoneVerified': True}), 200
    except Exception:
        app.logger.exception('Phone verification status update failed')
        return jsonify({'error': 'Could not save phone verification status.'}), 500


@app.post('/api/auth/phone-pin/reset')
def phone_pin_reset():
    """Reset a forgotten PIN for a phone+PIN account.

    The caller must present a Firebase ID token obtained by completing
    Firebase's own Phone Auth OTP flow (signInWithPhoneNumber) for the exact
    phone number on the account -- Firebase Phone Auth signs into the *same*
    uid for a phone number that already has an account, so a successful OTP
    verification here proves phone ownership without needing the old PIN.
    Admin accounts are excluded, mirroring phone-pin/login.
    """
    ok, detail = _require_user_bearer()
    if not ok:
        return detail
    ...
    if not re.fullmatch(r'\d{6,8}', pin):
        return jsonify({'error': 'PIN must contain 6 to 8 digits.'}), 400
    uid = str(detail.get('uid') or '').strip()
    phone_number = str(detail.get('phone_number') or '').strip()
    if not uid or not phone_number:
        return jsonify({'error': 'This reset requires a phone-verified sign-in.'}), 400
    # V31.108 audit fix: a `phone_number` claim alone does not prove the
    # caller actually completed Firebase's Phone Auth OTP flow - some sign-in
    # methods (e.g. a linked provider) can carry a phone_number claim without
    # it having just been freshly OTP-verified. firebase.sign_in_provider
    # records which auth method produced *this* ID token, so requiring it to
    # be 'phone' confirms the token itself came from the OTP flow this reset
    # is meant to rely on.
    sign_in_provider = str((detail.get('firebase') or {}).get('sign_in_provider') or '')
    ...
    try:
        ...
        is_admin = data.get('isAdmin') is True or role == 'admin'
        if is_admin:
            # Same rule as phone-pin/login: a PIN must never become an
            # alternate path into an admin-privileged session.
            return jsonify({'error': 'Admin accounts must reset their password by email.'}), 403
        if role not in {'student', 'teacher', 'parent'}:
            return jsonify({'error': 'This account has no valid BMT role.'}), 403
        now = datetime.now(timezone.utc)
        db.collection('authSecrets').document(uid).set({
            'pinHash': _hash_pin(pin), 'failedAttempts': 0, 'lockedUntil': None, 'updatedAt': now,
        }, merge=True)
        return jsonify({'success': True}), 200
    except RuntimeError as exc:
        return jsonify({'error': str(exc), 'requestId': getattr(g, 'request_id', '')}), 503
    except Exception:
        app.logger.exception('Phone/PIN reset failed')
        return jsonify({'error': 'Could not reset the PIN. Please try again.'}), 500
...
PAYMENT_PROVIDERS = {"telebirr", "cbe", "manual", "chapa"}
...
def _chapa_secret_key():
    return os.getenv("CHAPA_SECRET_KEY", "").strip()


def _public_base_url():
    configured = os.getenv("BMT_BASE_URL", "").strip().rstrip("/")
    if configured:
        return configured
    # Render terminates TLS at its edge and forwards the request to the
    # Gunicorn process over HTTP.  Build externally visible webhook URLs
    # from the trusted forwarded scheme instead of request.url_root, which
    # can otherwise produce an http:// Telegram/Chapa callback URL.
...
def _chapa_signature_ok(raw_body):
    ...
    return any(v and hmac.compare_digest(expected, v.strip()) for v in candidates)


def _chapa_json_request(method, url, **kwargs):
    key = _chapa_secret_key()
    if not key:
        raise RuntimeError("Chapa is not configured on the server.")
    ...
    try:
        ...
        data = {}
    if not response.ok:
        message = data.get("message") or data.get("error") or "Chapa request failed."
        raise RuntimeError(str(message))
    return data


def _chapa_verify_transaction(tx_ref):
    tx_ref = str(tx_ref or "").strip()
    if not tx_ref:
        raise ValueError("Chapa transaction reference is required.")
    return _chapa_json_request("GET", f"{CHAPA_API_BASE}/transaction/verify/{tx_ref}")


def _extract_chapa_data(payload):
    data = payload.get("data") if isinstance(payload, dict) else None
    return data if isinstance(data, dict) else (payload if isinstance(payload, dict) else {})


def _approve_verified_chapa_payment(db, tx_ref, verification_payload):
    from firebase_admin import firestore
    payment_id = _payment_id("chapa", tx_ref)
    payment_ref = db.collection("payments").document(payment_id)
    snap = payment_ref.get()
    if not snap.exists:
        raise ValueError("Chapa payment reference not found in BMT.")
    payment = snap.to_dict() or {}
    data = _extract_chapa_data(verification_payload)
    status = str(data.get("status") or verification_payload.get("status") or "").strip().lower()
    if status not in {"success", "successful", "paid", "completed"}:
        return {"unlocked": False, "status": status or "unknown", "paymentId": payment_id}

    expected_amount = Decimal(str(payment.get("amount", "0")))
    received_amount_raw = data.get("amount")
    if received_amount_raw is None:
        raise ValueError("Chapa verification did not return the payment amount.")
    received_amount = Decimal(str(received_amount_raw))
    if abs(received_amount - expected_amount) > Decimal("0.01"):
        raise ValueError("Chapa payment amount mismatch.")

    expected_currency = (os.getenv("PAYMENT_CURRENCY", "ETB") or "ETB").strip().upper()
    received_currency = str(data.get("currency") or "").strip().upper()
    if received_currency and received_currency != expected_currency:
        raise ValueError("Chapa payment currency mismatch.")

    result = _approve_payment_transaction(db, payment_id, "chapa", "chapa")
    if not result.get("already_approved"):
        _create_notification(
            db,
            result.get("uid"),
            "Payment verified 🎉",
            "Your Chapa payment was verified automatically. Premium access is now unlocked.",
            "payment",
            {"paymentId": payment_id, "provider": "chapa", "txRef": tx_ref},
        )
    return {"unlocked": True, "status": "success", "paymentId": payment_id, "alreadyProcessed": result.get("already_approved", False)}


@app.post('/api/payments/chapa/initialize')
def initialize_chapa_payment():
    """Create a server-controlled Chapa checkout. Amount/plan never come from the browser."""
    ok, detail = _require_user_bearer()
    if not ok:
        return detail
    try:
        ...
        # Restores the original early static validation (no DB/network
        # needed) that existed before the challenge_entry branch was added,
        # so an unrecognised plan is still rejected before the Chapa
        # configuration check runs, exactly as before.
        if plan_name != "challenge_entry" and plan_name not in PAYMENT_PLANS:
            return jsonify({"error": "Invalid payment plan."}), 400
        if not _chapa_secret_key():
            return jsonify({"error": "Chapa payment is not configured yet."}), 503

        db = firestore.client()
        uid = detail.get("uid", "")
        requested_child_uid = str(body.get("studentUid") or "").strip()
        if requested_child_uid and requested_child_uid != uid:
            actor = db.collection("users").document(uid).get()
            actor_data = actor.to_dict() if actor.exists else {}
            if str(actor_data.get("accountType") or actor_data.get("role") or "").lower() != "parent":
                return jsonify({"error": "Only a parent can pay for another student."}), 403
            links = db.collection("parentChildLinks").where("parentUid", "==", uid).limit(100).stream()
            authorized = any((x.to_dict() or {}).get("childUid") == requested_child_uid and (x.to_dict() or {}).get("status") == "verified" for x in links)
            if not authorized:
                return jsonify({"error": "You are not authorized to pay for this student."}), 403
            uid = requested_child_uid

        # V31.108 Competition Payment Flow: same additive challenge_entry
        # branch as /api/payments/submit above - amount comes from the
        # challenge's own entryFee, never PAYMENT_PLANS, never the browser.
        chapa_meta_extra = {}
        checkout_description = None
        if plan_name == "challenge_entry":
            challenge_id = str(body.get("challengeId", "")).strip()[:150]
            if not challenge_id:
                return jsonify({"error": "challengeId is required for a challenge_entry payment."}), 400
            csnap = db.collection("academicChallenges").document(challenge_id).get()
            if not csnap.exists:
                return jsonify({"error": "Challenge not found."}), 404
            challenge = csnap.to_dict() or {}
            try:
                fee = float(challenge.get("entryFee", 0) or 0)
            except (TypeError, ValueError):
                return jsonify({"error": "Invalid challenge entry fee configuration."}), 500
            if fee <= 0:
                return jsonify({"error": "This challenge is free and does not require payment."}), 409
            amount = fee
            payment_currency = str(challenge.get("entryCurrency") or "ETB")[:10].upper()
            chapa_meta_extra = {"challengeId": challenge_id}
            checkout_description = f"BMT competition entry fee: {str(challenge.get('title') or 'Academic Challenge')[:80]}"
        elif plan_name in PAYMENT_PLANS:
            amount = PAYMENT_PLANS[plan_name]["amount"]
            payment_currency = (os.getenv("PAYMENT_CURRENCY", "ETB") or "ETB").strip().upper()
            checkout_description = f"BMT {plan_name} premium subscription"
        else:
            return jsonify({"error": "Invalid payment plan."}), 400

        user_snap = db.collection("users").document(uid).get()
        user = user_snap.to_dict() if user_snap.exists else {}
        email = str(detail.get("email") or user.get("email") or "").strip()
        full_name = str(user.get("name") or detail.get("name") or "BMT Student").strip()
        parts = full_name.split(None, 1)
        first_name = (parts[0] if parts else "BMT")[:80]
        last_name = (parts[1] if len(parts) > 1 else "Student")[:80]
        phone = str(user.get("phone") or user.get("phoneNumber") or "").strip()
        if phone.startswith("+251") and len(phone) == 13:
            phone = "0" + phone[4:]
        if phone and not re.fullmatch(r"0[79]\d{8}", phone):
            phone = ""

        tx_ref = f"BMT-{uid[:10]}-{secrets.token_urlsafe(12).replace('-', '').replace('_', '')[:16]}"
        payment_id = _payment_id("chapa", tx_ref)
        now = _server_now()
        db.collection("payments").document(payment_id).set({
            "uid": uid,
            "studentName": full_name[:120],
            "provider": "chapa",
        ...
        payload = {
            ...
            "last_name": last_name,
            "tx_ref": tx_ref,
            "callback_url": f"{_public_base_url()}/api/payments/chapa/callback",
            "return_url": f"{_public_base_url()}/{'parent' if requested_child_uid else 'student'}?payment=return&tx_ref={tx_ref}",
            "customization": {
                "title": "Bright Mind Tutor",
            ...
            "meta": {
                ...
                "uid": uid,
                "plan": plan_name,
                "payment_reason": checkout_description,
                **chapa_meta_extra,
            },
        }
        if email:
        ...
        response = _chapa_json_request("POST", f"{CHAPA_API_BASE}/transaction/initialize", json=payload)
        data = _extract_chapa_data(response)
        checkout_url = str(data.get("checkout_url") or response.get("checkout_url") or "").strip()
        if not checkout_url:
            db.collection("payments").document(payment_id).update({"status": "InitializationFailed", "updatedAt": _server_now()})
            return jsonify({"error": "Chapa did not return a checkout URL."}), 502
        db.collection("payments").document(payment_id).update({"checkoutUrl": checkout_url, "updatedAt": _server_now()})
        return jsonify({"success": True, "paymentId": payment_id, "txRef": tx_ref, "checkoutUrl": checkout_url}), 200
    except RuntimeError as exc:
        return jsonify({"error": str(exc)}), 503
    except Exception:
        app.logger.exception("Chapa payment initialization failed")
        return jsonify({"error": "Could not start Chapa payment.", "requestId": getattr(g, "request_id", "")}), 502


@app.get('/api/payments/chapa/callback')
def chapa_payment_callback():
    tx_ref = str(request.args.get("tx_ref") or request.args.get("trx_ref") or "").strip()
    if not tx_ref:
        return jsonify({"error": "Missing transaction reference."}), 400
    try:
        from firebase_admin import firestore
        verification = _chapa_verify_transaction(tx_ref)
        result = _approve_verified_chapa_payment(firestore.client(), tx_ref, verification)
        return jsonify({"success": True, **result}), 200
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except Exception:
        app.logger.exception("Chapa callback verification failed")
        return jsonify({"error": "Could not verify Chapa payment."}), 502


@app.post('/api/payments/webhook/chapa')
def chapa_payment_webhook():
    raw = request.get_data(cache=True)
    if not _chapa_signature_ok(raw):
        return jsonify({"error": "Invalid Chapa webhook signature."}), 401
    try:
        from firebase_admin import firestore
        payload = request.get_json(silent=True) or {}
        ...
        # Chapa recommends re-querying the verification endpoint before granting value.
        verification = _chapa_verify_transaction(tx_ref)
        result = _approve_verified_chapa_payment(firestore.client(), tx_ref, verification)
        return jsonify({"success": True, **result}), 200
    except ValueError as exc:
        ...
        app.logger.exception("Chapa webhook processing failed")
...
def submit_payment():
    ...
    try:
        ...
        db = firestore.client()
        # V31.108 Competition Payment Flow: a second, additive plan type
        # whose amount is NOT looked up from the static PAYMENT_PLANS table
        # (subscription tiers only) but from the specific challenge's own
        # admin-configured entryFee/entryCurrency - the amount a browser
        # sends is still never trusted either way. Every existing plan in
        # PAYMENT_PLANS (monthly/yearly) behaves exactly as before; this is
        # a parallel branch, not a modification of that path.
        extra_fields = {}
        if plan_name == "challenge_entry":
            challenge_id = str(body.get("challengeId", "")).strip()[:150]
            if not challenge_id:
                return jsonify({"error": "challengeId is required for a challenge_entry payment."}), 400
            cref = db.collection("academicChallenges").document(challenge_id)
            csnap = cref.get()
            if not csnap.exists:
                return jsonify({"error": "Challenge not found."}), 404
            challenge = csnap.to_dict() or {}
            ...
            extra_fields = {"challengeId": challenge_id}
        elif plan_name in PAYMENT_PLANS:
            amount = PAYMENT_PLANS[plan_name]["amount"]
            currency = (os.getenv("PAYMENT_CURRENCY", "ETB") or "ETB").strip().upper()
        else:
            return jsonify({"error": "Invalid payment plan."}), 400
        ...
        ref.set({
            ...
            "payerName": student_name or "Student",
            "payerRole": "student",
...
@app.post('/api/payments/parent-submit')
def parent_submit_payment():
    ok, detail = _require_user_bearer()
    if not ok:
        return detail
    try:
        from firebase_admin import firestore
        body = request.get_json(silent=True) or {}
        parent_uid = detail.get('uid', '')
        child_uid = str(body.get('studentUid', '')).strip()
        plan_name = str(body.get('plan', 'monthly')).strip().lower()
        provider = str(body.get('provider', 'manual')).strip().lower()
        transaction_id = str(body.get('transactionId', '')).strip()[:160]
        if not child_uid or plan_name not in PAYMENT_PLANS or provider not in PAYMENT_PROVIDERS or not transaction_id:
            return jsonify({'error': 'Student, valid plan, provider and transaction ID are required.'}), 400
        db = firestore.client()
        links = db.collection('parentChildLinks').where('parentUid', '==', parent_uid).limit(100).stream()
        authorized = any((x.to_dict() or {}).get('childUid') == child_uid and (x.to_dict() or {}).get('status') == 'verified' for x in links)
        if not authorized:
            return jsonify({'error': 'You are not authorized to pay for this student.'}), 403
        student_snap = db.collection('users').document(child_uid).get()
        if not student_snap.exists:
            return jsonify({'error': 'Student account not found.'}), 404
        student = student_snap.to_dict() or {}
        payment_id = _payment_id(provider, transaction_id)
        ref = db.collection('payments').document(payment_id)
        existing = ref.get()
        if existing.exists:
            old = existing.to_dict() or {}
            if old.get('uid') != child_uid or old.get('payerUid') != parent_uid:
                return jsonify({'error': 'This transaction ID is already associated with another payment.'}), 409
            return jsonify({'success': True, 'paymentId': payment_id, 'status': old.get('status', 'Pending')}), 200
        now = _server_now()
        ref.set({
            'uid': child_uid, 'payerUid': parent_uid, 'payerRole': 'parent',
            'studentName': str(student.get('name') or student.get('displayName') or 'Student')[:120],
            'payerName': str((db.collection('users').document(parent_uid).get().to_dict() or {}).get('name') or (db.collection('users').document(parent_uid).get().to_dict() or {}).get('displayName') or 'Parent')[:120],
            'payerRole': 'parent',
            'className': str(student.get('className') or student.get('class') or student.get('grade') or '')[:20],
            'provider': provider, 'transactionId': transaction_id, 'plan': plan_name,
            'amount': PAYMENT_PLANS[plan_name]['amount'], 'currency': (os.getenv('PAYMENT_CURRENCY', 'ETB') or 'ETB').strip().upper(), 'timestamp': now, 'status': 'Pending',
        })
        return jsonify({'success': True, 'paymentId': payment_id, 'status': 'Pending'}), 201
    except Exception:
        app.logger.exception('Parent payment submission failed')
        return jsonify({'error': 'Parent payment could not be submitted.', 'requestId': getattr(g, 'request_id', '')}), 500
...
def reject_payment():
    ...
    try:
        ...
        ref = db.collection("payments").document(payment_id)
        transaction = db.transaction()
        body_reason = str(body.get("reason", "")).strip()[:500]
        admin_uid = detail.get("uid", "admin")
        ...
        def _run(transaction):
            ...
            status = str(payment.get("status", "Pending"))
            if status in {"Approved", "Verified"}:
                raise PermissionError("An approved payment cannot be rejected.")
            if status == "Rejected":
                return True
            now = _server_now()
            update_data = {
            ...
            if body_reason:
                update_data["rejectionReason"] = body_reason
            transaction.update(ref, update_data)
            return False

        already_rejected = _run(transaction)
        if already_rejected:
            return jsonify({"success": True, "status": "Rejected", "already_rejected": True}), 200
        _record_admin_audit(db, detail, 'payment_rejected', payment_id)
        return jsonify({"success": True, "status": "Rejected", "reason": body_reason})
...
def get_student_quizzes():
    ...
    try:
        ...
        try:
            ...
            entitlement = {}
        from targeting_access import content_visible_or_untargeted
        requested_grade = class_name
        if not is_admin:
            own_grade = str(user.get('class') or user.get('grade') or '').strip()
            if not own_grade or requested_grade != own_grade:
                return jsonify({'quizzes': []}), 200
        rows = []
        query = db.collection('quizzes').where('className', '==', requested_grade)
        for snap in query.stream():
            quiz = snap.to_dict() or {}
            if not is_admin and not content_visible_or_untargeted(user, quiz, entitlement):
                continue
            rows.append({
                'id': snap.id,
...
def get_student_videos():
    ...
    try:
        ...
        for snap in db.collection('videos').where('className', '==', class_name).stream():
            ...
            if not is_admin and not content_visible_or_untargeted(user, video, entitlement):
                continue
...
def _require_user_bearer():
    ...
    try:
        ...
        return False, _auth_error('Invalid authentication token.', 401)
    suspend_detail = _check_not_suspended(decoded)
    if suspend_detail is not None:
        return False, suspend_detail
    # Preview tokens are intentionally read-only. Enforce this server-side so
    # a preview cannot perform writes even if frontend safeguards are bypassed.
    if decoded.get('previewSession') is True and request.method not in {'GET', 'HEAD', 'OPTIONS'}:
        return False, _auth_error('Preview sessions are read-only.', 403)
    return True, decoded


def _check_not_suspended(decoded):
    """Shared suspension gate for _require_user_bearer(). A suspended
    account's Firestore doc carries isSuspended=True even though its
    Firebase Auth ID token is still perfectly valid (suspension is an
    admin toggle, not a credential change) - this is the one place that
    actually enforces it, and every register_x_routes(...) module was
    already passed this same _require_user_bearer function object, so
    fixing it here covers every route in the app, not just app.py's own.
    Returns None if the account may proceed, or an (response, status)
    tuple (via _auth_error) if it may not.

    Fails CLOSED on security-check failures. A missing uid claim is treated
    as an invalid authenticated identity, and a Firestore lookup failure is
    treated as a temporary service failure rather than silently allowing the
    request to continue without a suspension check.
    """
    try:
        uid = decoded.get('uid') or decoded.get('user_id') or ''
        if not uid:
            return _auth_error('Invalid authentication identity.', 401)
        from firebase_admin import firestore
        snap = firestore.client().collection('users').document(uid).get()
        user_data = snap.to_dict() if snap.exists else {}
        if not user_data.get('isSuspended'):
            return None
        reason = str(user_data.get('suspendReason') or '').strip()
        message = 'Your account has been suspended.'
        if reason:
            message += f' Reason: {reason}.'
        message += ' Contact support if you believe this is a mistake.'
        return _auth_error(message, 403)
    except Exception:
        app.logger.exception('Suspension status check failed')
        return _auth_error('Account status could not be verified. Please try again.', 503)
...
register_learning_challenge_routes(app, _require_user_bearer, _firebase_admin_from_env)
from progression_routes import register_progression_routes
register_progression_routes(app, _require_user_bearer, _firebase_admin_from_env)
from qualification_policy_routes import register_qualification_policy_routes
register_qualification_policy_routes(app, _require_user_bearer, _firebase_admin_from_env)
from awards_routes import register_award_routes
register_award_routes(app, _require_user_bearer, _firebase_admin_from_env)
...
register_library_routes(app, _require_user_bearer, _require_admin_bearer, _firebase_admin_from_env)
...
# V31.45 additive challenge-entry/payment verification bridge; does not modify payment routes.
from challenge_entry_routes import register_challenge_entry_routes
register_challenge_entry_routes(app, _require_user_bearer, _firebase_admin_from_env)

# V31.42 additive award ledger; isolated from A-owned UI/payment modules.
from award_ledger_routes import register_award_ledger_routes
register_award_ledger_routes(app, _require_user_bearer, _firebase_admin_from_env)

# V31.43 additive certificate issuance/verification; server-authoritative.
from certificate_routes import register_certificate_routes
register_certificate_routes(app, _require_user_bearer, _firebase_admin_from_env)

# V31.108 Competition Center admin completion: additive admin-only
# participants/analytics/certificates views. No new collections; reuses
# academicChallenges/challengeAttempts/challengeEntries/awardLedger/users.
from competition_admin_routes import register_competition_admin_routes
register_competition_admin_routes(app, _require_user_bearer, _firebase_admin_from_env)

# V31.85 Admin-manageable video/book media for the Student & Parent Development
# Center lessons (Psychology/Child Development). Additive - existing text
# content in parent_routes.py is unchanged; this only adds media attachments.
from development_content_routes import register_development_content_routes
register_development_content_routes(app, _require_admin_bearer, _require_user_bearer, _firebase_admin_from_env)

# V31.87 Student profile completion (age/school/guardian/education level) and
# the admin per-student detail/history view. Additive to existing profile fields.
from student_profile_routes import register_student_profile_routes
register_student_profile_routes(app, _require_user_bearer, _require_admin_bearer, _firebase_admin_from_env)

from student_course_routes import register_student_course_routes
register_student_course_routes(app, _require_user_bearer, _firebase_admin_from_env)

# V31.108 Interactive Lesson Builder: reusable content blocks on top of the
# existing courses/units/lessons (teacher authoring + student rendering).
from lesson_builder_routes import register_lesson_builder_routes
register_lesson_builder_routes(app, _require_user_bearer, _firebase_admin_from_env)

# Student Attendance (Regular classroom attendance only - Distance students
# use course/lesson progress instead; see attendance_routes.py docstring).
from attendance_routes import register_attendance_routes
register_attendance_routes(app, _require_user_bearer, _firebase_admin_from_env)

# Student Mark / Grade List - read-only aggregation of the existing
# Assignment/Quiz/Exam grading data; see marklist_routes.py docstring.
from marklist_routes import register_marklist_routes
register_marklist_routes(app, _require_user_bearer, _firebase_admin_from_env)

# V31.64 additive Telegram Academic Challenge integration; isolated from Cloud-B library modules.
from telegram_challenge_routes import register_telegram_challenge_routes
register_telegram_challenge_routes(app, _firebase_admin_from_env)

# V31.73-B Three-Bot Telegram architecture: Moderator/Support bot, Publisher/Content
# bot, and the admin multi-channel publishing platform. Each bot is a separate
# Telegram identity with its own token/webhook secret; the Academic Challenge bot
# above is unaffected.
from telegram_platform_routes import register_telegram_platform_routes
register_telegram_platform_routes(app, _require_user_bearer, _require_admin_bearer, _firebase_admin_from_env)
from telegram_publisher_bot_routes import register_telegram_publisher_bot_routes
register_telegram_publisher_bot_routes(app, _firebase_admin_from_env, _require_user_bearer)
from telegram_moderator_bot_routes import register_telegram_moderator_bot_routes
register_telegram_moderator_bot_routes(app, _firebase_admin_from_env)


# V31.75 Admin Telegram Bot control panel. Secrets remain server-side.
...
def admin_telegram_status():
    ...
    if not token:
        return jsonify({'success': True, 'configured': False, 'webhookConfigured': False})
    try:
        base = f'https://api.telegram.org/bot{token}'
        me = requests.get(base + '/getMe', timeout=10)
        wi = requests.get(base + '/getWebhookInfo', timeout=10)
        md = me.json() if me.content else {}
        wd = wi.json() if wi.content else {}
        bot = md.get('result') or {}
        webhook = wd.get('result') or {}
        return jsonify({
            'success': True,
            'configured': bool(md.get('ok')),
            'botName': bot.get('first_name') or '',
            'username': bot.get('username') or '',
            'webhookConfigured': bool(webhook.get('url')),
            'webhookUrl': webhook.get('url') or '',
            'webhookPendingUpdates': webhook.get('pending_update_count', 0),
            'secretConfigured': bool(secret),
        })
    except requests.RequestException:
        return jsonify({'error':'Telegram API is temporarily unavailable.'}), 503

@app.post('/api/admin/telegram/set-webhook')
def admin_telegram_set_webhook():
    ok, detail = _require_admin_bearer()
    if not ok: return detail
    import os, requests
    token = os.getenv('TELEGRAM_BOT_TOKEN', '').strip()
    ...
    if not token or not secret:
        return jsonify({'error':'TELEGRAM_BOT_TOKEN and TELEGRAM_WEBHOOK_SECRET must be configured on the server.'}), 503
    body = request.get_json(silent=True) or {}
    supplied = str(body.get('url') or '').strip()
    if supplied:
        from urllib.parse import urlparse
        parsed = urlparse(supplied)
        if parsed.scheme != 'https' or not parsed.netloc or parsed.path != '/api/telegram/webhook':
            return jsonify({'error':'Webhook URL must be HTTPS and end exactly with /api/telegram/webhook.'}), 400
        webhook_url = supplied
    else:
        webhook_url = _public_base_url() + '/api/telegram/webhook'
        if not webhook_url.startswith('https://'):
            return jsonify({'error':'Public HTTPS URL is required. Enter the deployed HTTPS webhook URL in the field.'}), 400
    try:
        r = requests.post(
            f'https://api.telegram.org/bot{token}/setWebhook',
            json={'url': webhook_url, 'secret_token': secret, 'allowed_updates': ['message', 'callback_query', 'inline_query']},
            timeout=15,
        )
        try:
            data = r.json() if r.content else {}
        ...
        if not r.ok or not data.get('ok'):
            telegram_error = (
                ...
                or (r.text[:500] if r.text else None)
                or f'Telegram API HTTP {r.status_code}'
            )
            app.logger.error('Telegram setWebhook failed: %s', telegram_error)
            return jsonify({'error': telegram_error, 'telegramError': telegram_error}), 502
        return jsonify({'success': True, 'webhookUrl': webhook_url})
    except requests.RequestException:
        return jsonify({'error':'Telegram API is temporarily unavailable.'}), 503

# V31.84 Admin control panel for the Moderator and Publisher Telegram bots.
# Mirrors the Academic bot's status/set-webhook pair above, but scoped to each
# bot's own token/secret/webhook path so the three bots stay fully independent.
def _telegram_bot_status(token_env, secret_env):
    ok, detail = _require_admin_bearer()
    if not ok: return detail
    import os, requests
...
def _telegram_bot_set_webhook(token_env, secret_env, webhook_path):
    ok, detail = _require_admin_bearer()
    if not ok: return detail
    import os, requests
    token = os.getenv(token_env, '').strip()
    secret = os.getenv(secret_env, '').strip()
    if not token or not secret:
        return jsonify({'error': f'{token_env} and {secret_env} must be configured on the server.'}), 503
    body = request.get_json(silent=True) or {}
    supplied = str(body.get('url') or '').strip()
    if supplied:
        from urllib.parse import urlparse
        ...
        if parsed.scheme != 'https' or not parsed.netloc or parsed.path != webhook_path:
            return jsonify({'error': f'Webhook URL must be HTTPS and end exactly with {webhook_path}.'}), 400
        webhook_url = supplied
    else:
        webhook_url = _public_base_url() + webhook_path
        if not webhook_url.startswith('https://'):
            return jsonify({'error': 'Public HTTPS URL is required. Enter the deployed HTTPS webhook URL in the field.'}), 400
    try:
        r = requests.post(
            f'https://api.telegram.org/bot{token}/setWebhook',
            json={'url': webhook_url, 'secret_token': secret, 'allowed_updates': ['message', 'callback_query']},
            timeout=15,
        )
        try:
            data = r.json() if r.content else {}
        ...
        if not r.ok or not data.get('ok'):
            ...
            app.logger.error('Telegram setWebhook failed for %s: %s', token_env, telegram_error)
            return jsonify({'error': telegram_error, 'telegramError': telegram_error}), 502
        return jsonify({'success': True, 'webhookUrl': webhook_url})
    except requests.RequestException:
        return jsonify({'error': 'Telegram API is temporarily unavailable.'}), 503

@app.get('/api/admin/telegram/moderator/status')
def admin_telegram_moderator_status():
    return _telegram_bot_status('TELEGRAM_MODERATOR_BOT_TOKEN', 'TELEGRAM_MODERATOR_WEBHOOK_SECRET')

@app.post('/api/admin/telegram/moderator/set-webhook')
def admin_telegram_moderator_set_webhook():
    return _telegram_bot_set_webhook('TELEGRAM_MODERATOR_BOT_TOKEN', 'TELEGRAM_MODERATOR_WEBHOOK_SECRET', '/api/telegram/moderator/webhook')

@app.get('/api/admin/telegram/publisher/status')
def admin_telegram_publisher_status():
    return _telegram_bot_status('TELEGRAM_PUBLISHER_BOT_TOKEN', 'TELEGRAM_PUBLISHER_WEBHOOK_SECRET')

@app.post('/api/admin/telegram/publisher/set-webhook')
def admin_telegram_publisher_set_webhook():
    return _telegram_bot_set_webhook('TELEGRAM_PUBLISHER_BOT_TOKEN', 'TELEGRAM_PUBLISHER_WEBHOOK_SECRET', '/api/telegram/publisher/webhook')

# B-owned Smart Quiz Scanner: additive OCR/question-extraction path; does not replace Library/Reader.
from smart_quiz_scanner_routes import register_smart_quiz_scanner_routes
register_smart_quiz_scanner_routes(app, _require_user_bearer, _require_admin_bearer, _firebase_admin_from_env)
