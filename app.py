...
BMT_VERSION = os.getenv("BMT_VERSION", "31.82").strip() or "31.82"
...
def phone_pin_register():
    ...
    try:
        ...
        profile = {
            ...
            'class': class_name, 'role': role, 'accountType': role,
...
def parent_submit_payment():
    ...
    try:
        ...
        ref.set({
            ...
            'amount': PAYMENT_PLANS[plan_name]['amount'], 'timestamp': now, 'status': 'Pending',
        })
        return jsonify({'success': True, 'paymentId': payment_id, 'status': 'Pending'}), 201
    except Exception:
...
def admin_telegram_set_webhook():
    ...
    if supplied:
        ...
        webhook_url = request.url_root.rstrip('/') + '/api/telegram/webhook'
    ...
    try:
        ...
        if not r.ok or not data.get('ok'):
            return jsonify({'error':'Telegram rejected the webhook configuration.'}), 502
