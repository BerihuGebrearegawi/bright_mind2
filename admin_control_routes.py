...
def register_admin_control_routes(app, require_admin, firebase_admin_factory):
    ...
    def count(database, collection_name, limit=5000):
        try:
            agg = database.collection(collection_name).count().get()
            return agg[0][0].value
        except Exception:
            app.logger.warning('Aggregation count failed for %s, falling back to stream count', collection_name)
    ...
    def admin_students():
        ...
        try:
            database = db()
            rows = []
            for snap in database.collection('users').limit(5000).stream():
                x = snap.to_dict() or {}
                ...
                rows.append({
                    ...
                    'phone': x.get('phone') or x.get('phoneNumber') or '',
                    'grade': str(x.get('className') or x.get('class') or x.get('grade') or ''),
                    'accountType': x.get('accountType') or 'student',
                    'isPaid': bool(x.get('isPaid')),
                    'isSuspended': bool(x.get('isSuspended')),
                    'registeredAt': iso(x.get('registeredAt') or x.get('createdAt')),
                    'lastLoginAt': iso(x.get('lastLoginAt') or x.get('lastSeenAt')),
                })
            rows.sort(key=lambda r: (str(r.get('name') or '').lower(), str(r.get('email') or '').lower()))
            return jsonify({'success': True, 'students': rows, 'count': len(rows)}), 200
        except Exception:
            app.logger.exception('Admin student list failed')
            return jsonify({'error': 'Unable to load registered students.'}), 500

    @app.get('/api/admin/preview-users')
    def admin_preview_users():
        """Real accounts an admin can choose to preview (student/teacher/parent).
        Backs the Dashboard Preview picker so previews show one specific real
        person's real data instead of fabricated placeholder content."""
        ok, detail = require_admin()
        if not ok:
            return detail
        ...
        try:
            ...
            for snap in database.collection('users').limit(5000).stream():
                ...
                if x.get('isAdmin'):
                    continue
                actual_role = str(x.get('role') or x.get('accountType') or '').lower()
                role_ok = actual_role == role or (role == 'student' and actual_role in {'', 'learner'})
                if not role_ok:
                    continue
                name = x.get('displayName') or x.get('name') or x.get('fullName') or 'Unnamed'
                email = x.get('email') or ''
                phone = x.get('phone') or x.get('phoneNumber') or ''
                if query and query not in name.lower() and query not in email.lower() and query not in phone.lower():
                    continue
                rows.append({
                    'uid': snap.id,
                    'name': name,
                    'email': email,
                    'phone': phone,
                    'label': str(x.get('className') or x.get('class') or x.get('subject') or ''),
                })
            rows.sort(key=lambda r: r['name'].lower())
            return jsonify({'success': True, 'users': rows[:50], 'count': len(rows)}), 200
        except Exception:
            app.logger.exception('Admin preview user list failed')
            return jsonify({'error': 'Unable to load accounts for preview.'}), 500
