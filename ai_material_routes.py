...
ALLOWED_LEARNING_MODES = {"Regular", "Distance"}
ALLOWED_AUDIENCES = {"Free Regular", "Paid Regular", "Distance", "Scholarship", "Competition"}


def _clean_targeting_list(raw, allowed):
    values = [str(x).strip() for x in raw] if isinstance(raw, list) else []
    return list(dict.fromkeys(x for x in values if x in allowed))
...
def register_ai_material_routes(app, require_user, firebase_admin_factory):
    ...
    def list_ai_materials():
        ...
        try:
            ...
            for d in docs:
                ...
                if x.get("storageProvider") == "gcs" and x.get("storagePath"):
                    # The stored sourceUrl was a short-lived Cloudinary signed
                    # link minted at ingestion time - mint a fresh one now
                    # rather than serving what is very likely an expired URL.
                    try:
                        from cloudinary_storage import signed_url
                        source_url = signed_url(x["storagePath"])
                    except Exception:
                        app.logger.exception("Could not refresh AI material source URL for %s", d.id)
                items.append({"id": d.id, "title": x.get("title"), "grade": x.get("grade"),
                              "subject": x.get("subject"), "sourceType": x.get("sourceType"),
                              "sourceUrl": source_url, "chunkCount": x.get("chunkCount", 0),
                              "active": x.get("active", True),
                              "learningModes": x.get("learningModes") or [],
    ...
    def create_ai_material():
        ...
        try:
            ...
            raw_modes = request.form.getlist('learningModes') or (request.get_json(silent=True) or {}).get('learningModes')
            raw_audiences = request.form.getlist('audiences') or (request.get_json(silent=True) or {}).get('audiences')
            learning_modes = _clean_targeting_list(raw_modes, ALLOWED_LEARNING_MODES)
            audiences = _clean_targeting_list(raw_audiences, ALLOWED_AUDIENCES)
            if isinstance(raw_modes, list) and raw_modes and not learning_modes:
                return jsonify({"error": "No valid learning mode supplied."}), 400
            if isinstance(raw_audiences, list) and raw_audiences and not audiences:
            ...
            if learning_modes:
                material_doc['learningModes'] = learning_modes
            if audiences:
                material_doc['audiences'] = audiences
            material_ref.set(material_doc)
            ...
            for i, content in enumerate(parts):
                ...
                if learning_modes:
                    chunk_doc['learningModes'] = learning_modes
                if audiences:
                    chunk_doc['audiences'] = audiences
                batch.set(ref, chunk_doc)
                if (i + 1) % 400 == 0:
