...
def _question_public(q):
    return {
        ...
        "learningModes": q.get("learningModes") or ["Regular"],
        "audiences": q.get("audiences") or ["Free Regular", "Paid Regular"],
    }
...
def _targeting(body):
    ...
    if not modes or not audiences:
        raise ValueError("At least one valid Learning Mode and Target Audience is required.")
    return modes, audiences
...
def _question_payload(body, uid):
    ...
    return {
        ...
        "createdBy": uid,
        "learningModes": _targeting(body)[0],
        "audiences": _targeting(body)[1],
    }
...
def _challenge_public(challenge):
    return {
        ...
        "learningModes": challenge.get("learningModes") or ["Regular"],
        "audiences": challenge.get("audiences") or ["Free Regular", "Paid Regular"],
        # V31.108 Competition Payment Flow: entryFee/entryCurrency already
        # existed server-side (configure_entry_fee in
        # challenge_entry_routes.py, enforced in start_challenge below) but
        # were never exposed here, so a student had no way to even see a
        # fee applied before this. Read-only, admin-set values.
        "entryFee": float(challenge.get("entryFee", 0) or 0),
        "entryCurrency": str(challenge.get("entryCurrency") or "ETB"),
    }
...
def register_learning_challenge_routes(app, require_user, firebase_admin_factory):
    ...
    def available_challenges():
        ...
        try:
            ...
            now = _utc_now(); rows = []
            # V31.108 Phase E fix: this used to filter status=="published" only.
            # /api/challenges/start flips a challenge to "active" the moment
            # the FIRST student starts it (a global field, not per-student) -
            # so without this fix, the instant one student began the
            # challenge it silently vanished from every other student's list
            # for the rest of its run. No student-facing UI existed before
            # Phase E to notice this.
            for snap in db.collection("academicChallenges").where("status", "in", ["published", "active"]).limit(100).stream():
                c = snap.to_dict() or {}; c["id"] = snap.id
                if not content_target_allowed(user, c, entitlement): continue
                ...
                row = _challenge_public(c)
                # V31.108 Competition Payment Flow: tells the student's own
                # UI whether they already have a server-verified entry, so
                # it can show "Start" vs "Pay to enter" without the client
                # ever deciding eligibility itself.
                if row["entryFee"] > 0:
                    verified = db.collection("challengeEntries").where("challengeId", "==", snap.id).where("userId", "==", uid).where("status", "==", "verified").limit(1).stream()
                    row["entryVerified"] = next(iter(verified), None) is not None
                else:
                    row["entryVerified"] = True
                rows.append(row)
    ...
    def start_challenge():
        ...
        try:
            ...
            if not content_target_allowed(user, c, entitlement):
                return jsonify({"error": "This challenge is not assigned to your learning mode or audience."}), 403
            ...
            # V31.108 Competition Center fix: GET /api/challenges already hid a
            # region/zone/woreda-restricted Regional Challenge from a student
            # outside that area, but this endpoint never re-checked it - a
            # student who obtained the challengeId any other way (a link
            # shared by a friend in the target region, a cached response, a
            # direct API call) could still start and submit it. Region/zone/
            # woreda are now enforced server-side here too, exactly mirroring
            # the listing's own filter, so hiding from the list is no longer
            # the only thing standing between an out-of-area student and a
            # Regional Challenge.
            student_region = str(user.get("region") or "").strip()
            student_zone = str(user.get("zone") or "").strip()
            student_woreda = str(user.get("woreda") or "").strip()
            ...
            if challenge_region and challenge_region != "ALL" and challenge_region != student_region:
                return jsonify({"error": "This challenge is not open to your region."}), 403
            challenge_zone = str(c.get("zone") or "").strip()
            if challenge_zone and challenge_zone != student_zone:
                return jsonify({"error": "This challenge is not open to your zone."}), 403
            challenge_woreda = str(c.get("woreda") or "").strip()
            if challenge_woreda and challenge_woreda != student_woreda:
                return jsonify({"error": "This challenge is not open to your woreda."}), 403
    ...
    @app.get("/api/challenges/mine")
    def my_challenge_attempts():
        """V31.108 Phase E: a student's own attempt history. Nothing before
        this endpoint let a student see their past results outside of a
        single just-submitted response - there was no student-facing
        Competition UI at all until this phase."""
        detail, error = _identity()
        if error: return error
        try:
            db = _db(); uid = detail["uid"]; role, _ = _role(db, uid, detail)
            if role != "student":
                return jsonify({"error": "Only student accounts can view their attempts."}), 403
            rows = []
            for snap in db.collection("challengeAttempts").where("userId", "==", uid).limit(200).stream():
                a = snap.to_dict() or {}
                if a.get("status") != "submitted":
                    continue
                challenge_snap = db.collection("academicChallenges").document(str(a.get("challengeId") or "")).get()
                title = (challenge_snap.to_dict() or {}).get("title", "Academic Challenge") if challenge_snap.exists else "Academic Challenge"
                rows.append({
                    "attemptId": snap.id, "challengeId": a.get("challengeId"), "challengeTitle": title,
                    "score": a.get("score", 0), "totalPoints": a.get("totalPoints", 0),
                    "percentage": a.get("percentage", 0), "status": a.get("statusResult", "FAIL"),
                    "submittedAt": _as_dt(a.get("submittedAt")).isoformat() if _as_dt(a.get("submittedAt")) else None,
                })
            rows.sort(key=lambda x: x.get("submittedAt") or "", reverse=True)
            return jsonify({"attempts": rows}), 200
        except Exception:
            app.logger.exception("My challenge attempts lookup failed")
            return jsonify({"error": "Unable to load your results."}), 500
